...
(function (root, factory) {
  ...
  function looksLikeAParser (p) {
    return p && typeof p === 'object' && isFunction(p.parse)
  }
  ...
  const unqualifiedKeywordRegex1 = /^:[a-zA-Z0-1]+$/
  const keywordRegex = /^:{1,2}[a-zA-Z0-1/]+$/

  // const tokenRe1 = new RegExp(tokenEnd)
  // const result888 = ':smile'.match(tokenRe1)

  // console.assert(result888, 'one')
  // console.assert(result888[0], 'two')
  // console.assert(result888[0] === ':smile', 'three')

  // parsers.token = Regex({ name: 'token', regex: new RegExp(unqualifiedKeywordRegex) })

  parsers.token = Choice({
    parsers: [
      // Regex({ name: 'token', regex: new RegExp(unqualifiedKeywordRegex1) }),
