...
(function (root, factory) {
  ...
  function parseNs (nodesArr) {
    ...
    while (idx < numNodes) {
      ...
      if (isParenOpener(node)) {
        ...
        if (insideImportPackageList) {
          ...
          insideNsForm = false
          // FIXME: we can assume that there is only one ns per file and exit the loop early here
          // optimization
        }
      ...
      const nodeContainsText2 = nodeContainsText(node)

      // collect the ns symbol
      if (idx > nsNodeIdx && nextTextNodeIsNsSymbol && isTokenNode2 && nodeContainsText2) {
      ...
      } else if (idx > inc(referClojureNodeIdx) && collectReferClojureExcludeSymbols && parenNestingDepth >= 3 && isTokenNode2 && nodeContainsText2 && result.referClojure && isArray(result.referClojure.exclude)) {
      ...
      } else if (idx > inc(referClojureNodeIdx) && collectReferClojureOnlySymbols && parenNestingDepth >= 3 && isTokenNode2 && nodeContainsText2 && result.referClojure && isArray(result.referClojure.only)) {
      ...
      } else if (idx > inc(referClojureNodeIdx) && collectReferClojureRenameSymbols && parenNestingDepth >= 3 && isTokenNode2 && nodeContainsText2 && result.referClojure && isArray(result.referClojure.rename)) {
      ...
      // collect the require :as
      } else if (idx > requireNodeIdx && insideRequireForm && nextTokenIsAsSymbol && isTokenNode2 && nodeContainsText2) {
      ...
      } else if (idx > requireNodeIdx && insideRequireForm && isTokenNode2 && nodeContainsText2) {
      ...
      // collect import packages + classes
      } else if (idx > importNodeIdx && insideImportForm && !insideImportPackageList && isTokenNode(node) && nodeContainsText(node)) {
        ...
        if (!isArray(result.importsObj[packageName])) {
          result.importsObj[packageName] = []
        }

        stackPush(result.importsObj[packageName], className)
        ...
        if (!importPackageListFirstToken) {
          ...
          stackPush(result.importsObj[importPackageListFirstToken], node.text)
        }
      }
    ...
    if (result.importsObj) {
      ...
      objectForEach(result.importsObj, function (packageName, classes) {
