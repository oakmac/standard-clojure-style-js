...
(function (root, factory) {
  ...
  function parseNs (nodesArr) {
    ...
    while (idx < numNodes) {
      ...
      if (idx > nsNodeIdx && nextTextNodeIsNsSymbol && isTokenNode(node) && isNonEmptyTextNode(node)) {
        result.nsSymbol = node.text
        nextTextNodeIsNsSymbol = false
        // collect the require :as
      } else if (idx > requireNodeIdx && insideRequireForm && nextTokenIsAsSymbol && isTokenNode(node) && isNonEmptyTextNode(node)) {
      ...
      } else if (idx > requireNodeIdx && insideRequireForm && isTokenNode(node) && isNonEmptyTextNode(node)) {
      ...
      } else if (idx > importNodeIdx && insideImportForm && !insideImportPackageList && isTokenNode(node) && isNonEmptyTextNode(node)) {
