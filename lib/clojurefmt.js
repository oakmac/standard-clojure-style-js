...
(function (root, factory) {
  ...
  // inserts itms (an Array) into arr (also an Array) at idx
  // returns arr with the new items added
  // NOTE: this method modifies arr
  function arrayInsert (arr, idx, itms) {
    // TODO: throw here is idx is out of range?
    const numItms = arraySize(itms)
    let i = 0
    while (i < numItms) {
      const itm = itms[i]
      const insertIdx = idx + i
      arr.splice(insertIdx, 0, itm)
      i = inc(i)
    }
    return arr
  }
  ...
  function Node (opts) {
    nodeIdCounter = inc(nodeIdCounter)
  ...
  parsers.braces = Seq({
    name: 'braces',
    parsers: [
      // NOTE: difference from cs_parser.py
      Choice({
        parsers: [
          Char({ name: '.open', char: '{' }),
  ...
  // NOTES:
  // Working with the flat array of nodes should be easier than working with the tree.

  // // recursively runs function f on every node in the tree
  // // assigns the return value of f(node) to node
  // // ie: modifying the tree
  // function recurseAllChildrenModifyInPlace (node, f) {
  //   f(node)
  //   if (node.children) {
  //     const numChildren = arraySize(node.children)
  //     let i = 0
  ...
  // function treeToFlatMap (tree) {
  //   const nodesMap = {}
  //   function addNodeToFlatMap (node) {
  //     nodesMap[node.id] = node
  //   }
  //   recurseAllChildren(tree, addNodeToFlatMap)
  //   return nodesMap
  // }
  ...
  // // returns the array index of the first node that matches isNodeFn()
  // // or -1 if not found
  // function findIdxOfNode (nodes, isNodeFn) {
  ...
  // returns an array of the node indexes that match isNodeFn
  // NOTE: this is a full O(n) operation
  // TODO: improve the performance here by requiring less scanning
  function findNodeIdxs (nodes, isNodeFn) {
    const foundIdxs = []
    const numNodes = arraySize(nodes)
    let idx = 0
    while (idx < numNodes) {
      const node = nodes[idx]
      if (isNodeFn(node)) {
        foundIdxs.push(idx) // TODO: convert to language neutral
      }
      idx = inc(idx)
    }
    return foundIdxs
  ...
  // returns the array index of the ns token, or -1 if no ns form is found
  // NOTE: this only works with one ns form per file
  // TODO: possible to make this faster by recording this index as we create the tree
  //       or convert from the tree to the nodes array
  function findIdxOfNsForm (nodes) {
    const numNodes = arraySize(nodes)
    let idx = 0
    while (idx < numNodes) {
      const node = nodes[idx]
  ...
  function isRequireNode (n) {
    return n.name === 'token' && (n.text === ':require' || n.text === 'require')
  }

  function isRequireChildVector (n) {
    return n.name === 'brackets' &&
    isArray(n.children) &&
    n.children[0] &&
    n.children[0].name === '.open' &&
    n.children[1] &&
    n.children[1].name === '.body'
  }

  function isValidRequireImportToken (n) {
    return n.name === 'token' && n.text !== '' && !isRequireNode(n)
  }

  // applies the "how to ns" rules to an array of nodes
  // returns the new array
  function applyHowToNsRules (nodes) {
    const nsIdx = findIdxOfNsForm(nodes)

    // skip these steps if the input does not have an ns form
    if (nsIdx === -1) {
      return nodes
    } else {
      const requireIdxs = findNodeIdxs(nodes, isRequireNode)
      const requireImportTokens = []
      let i = 0
      const numRequireNodes = arraySize(requireIdxs)
      while (i < numRequireNodes) {
        const requireNodeIdx = requireIdxs[i]
        const requireNodeParent = nodes[dec(requireNodeIdx)]

        let j = 0
        const numRequireChildren = arraySize(requireNodeParent.children)
        while (j < numRequireChildren) {
          const childNode = requireNodeParent.children[j]

          // add a single token node
          if (isValidRequireImportToken(childNode)) {
            requireImportTokens.push({
              text: childNode.text
            })
          } else if (isRequireChildVector(childNode)) {
            const childVector = childNode.children[1].children
            if (childVector[0] && isValidRequireImportToken(childVector[0])) {
              requireImportTokens.push({
                text: childVector[0].text
              })
            }
          }
      ...
      console.log('requireImportTokens:', requireImportTokens)

      // Thoughts:
      // - "modify in place" is turning out to be quite tricky
      // - might be best to do a single pass through the ns form and build up a data structure
      //   that can be converted into the ideal result
      // - need to think about how reader conditionals would work in this scenario
      // - also comment lines that are "attached" to a require

      // Rule: Keywords or symbols?
      // https://stuartsierra.com/2016/clojure-how-to-ns.html#keywords-or-symbols
      // NOTE: this breaks the "start" and "end" properties of all subsequent nodes
      // nodes[requireIdx].text = ':require'

      // TODO: combine multiple :require statements into one
      // TODO: convert require, import, refer-clojure symbols into keywords
      // TODO: convert parens to vectors for :require
      // TODO: convert vectors to parens for :import
      // TODO: sort the main three clauses
      // TOOD: sort the :require list
      // TODO: convert single tokens to vectors
  ...
  // parses inputTxt (Clojure code) and returns a formatted String of it
  function format (inputTxt) {
    const tree = parse(inputTxt)

    // const nodesMap = treeToFlatMap(tree)
    ...
    const nodesArr0 = flattenTree(tree)
    // nodesArr1 = nodes with "how to ns" adjustments
    const nodesArr1 = applyHowToNsRules(nodesArr0)
    const numNodes = arraySize(nodesArr1)
    ...
    while (idx < numNodes) {
      // let prevNode = null
      // if (i > 0) { prevNode = nodesArr1[dec(i)] }
      const node = nodesArr1[idx]
      const nextTextNode = nextNodeWithText(nodesArr1, inc(idx))
      ...
      if (currentNodeIsNewline) {
        ...
        const parenTrailClosers = findCloserNodes(nodesArr1, inc(idx))
