...
(function (root, factory) {
  ...
  function parseNs (nodesArr) {
    ...
    while (idx < numNodes) {
      ...
      if (dec(idx) >= 0) {
        prevNode = nodesArr[dec(idx)]
        if (isNewlineNode(prevNode)) {
          prevNodeIsNewline = true
        }
      }
      ...
      // collect comments at the end of a line
        ...
        if (insideRequireForm && activeRequireIdx < 0) {
          result.requireCommentAfter = commentAtEndOfLine
        } else if (insideRequireForm && activeRequireIdx >= 0) {
          result.requires[activeRequireIdx].commentAfter = commentAtEndOfLine
        } else if (insideReferClojureForm && referClojureNodeIdx < 0) {
          // FIXME: attach comment to referClojure form
        } else if (insideReferClojureForm && referClojureNodeIdx >= 0) {
          // FIXME: attach comment to referClojure form
        } else if (insideImportForm && !insideImportPackageList && !result.importsObj) {
