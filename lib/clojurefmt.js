...
(function (root, factory) {
  ...
  function isRequireMacrosNode (node) {
  ...
  function isReferMacrosNode (node) {
  ...
  function parseNs (nodesArr) {
    ...
    while (idx < numNodes) {
      ...
      } else if (insideNsForm && isRequireMacrosNode(node)) {
      ...
      // is this :require :as ?
      } else if (idx > requireNodeIdx && insideRequireForm && isTokenNode2 && isAsNode(node)) {
      ...
      // collect :require-macros :as symbols
      ...
      } else if (insideRequireMacrosForm && idx > requireMacrosNodeIdx && isReferNode(node)) {
        requireMacrosReferNodeIdx = idx

      // :require-macros :as
      } else if (insideRequireMacrosForm && idx > requireMacrosNodeIdx && isAsNode(node)) {
      ...
      // collect :require-macros symbol
        ...
        if (result.requireMacros[activeRequireMacrosIdx]) {
          console.log('the thing exists:', node.text)
        } else {
          const obj = {
            symbol: node.text
          }

          stackPush(result.requireMacros, obj)
          activeRequireMacrosIdx = inc(activeRequireMacrosIdx)

          if (insideReaderConditional && currentReaderConditionalPlatform) {
            result.requireMacros[activeRequireMacrosIdx].platform = currentReaderConditionalPlatform
          }

          // FIXME: add commentsAbove here
        }

        // console.log("hhhhhhh:", node.text)
      ...
      } else if (idx > requireNodeIdx && insideRequireForm && isTokenNode2 && isReferMacrosNode(node)) {
        referMacrosIdx = idx
  ...
  function formatNs (ns) {
      ...
      // use splicing reader conditional #?@(
        ...
        while (platformIdx < numPlatforms) {
          ...
          while (reqIdx2 < numFilteredReqs) {
            ...
            } else if (isLastRequireForThisPlatform && req.commentAfter && requireIsLastMainForm) {
    ...
    if (numImports > 0) {
      ...
      // imports that are not specific to a platform
      ...
      // FIXME: support comments on the last import line here
