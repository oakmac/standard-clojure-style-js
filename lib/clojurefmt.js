...
(function (root, factory) {
  ...
  function isOneSpaceOpener (opener) {
    return opener.text === '{' || opener.text === '[' || opener.text === '#{'
  }

  // returns the number of spaces to use for indentation at the beginning of a line
  function numSpacesForIndentation (parenDepth, wrappingOpener) {
    if (isOneSpaceOpener(wrappingOpener)) {
      return inc(wrappingOpener.colIdx)
    } else {
      return parenDepth * 2
    }
  }
  ...
  function format (inputTxt) {
    ...
    while (idx < numNodes) {
      ...
      if (isOpener(node)) {
        ...
        const nodeWithColIdx = node
        nodeWithColIdx.colIdx = colIdx

        parenStack.push(nodeWithColIdx) // TODO: abstract to language neutral
      ...
      if (currentNodeIsNewline) {
        ...
        if (inc(idx) < numNodes) {
          ...
          const numSpaces = numSpacesForIndentation(parenNestingDepth, topOfTheParenStack)
