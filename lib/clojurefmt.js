...
(function (root, factory) {
  ...
  function parseNs (nodesArr) {
    ...
    while (idx < numNodes) {
      ...
      if (isParenOpener(node)) {
        ...
        if (collectReferClojureOnlySymbols && parenNestingDepth < 3) {
      ...
      } else if (idx > inc(referClojureNodeIdx) && collectReferClojureOnlySymbols && parenNestingDepth >= 3 && isTokenNode2 && nodeContainsText2 && result.referClojure && isArray(result.referClojure.exclude)) {
  ...
  function formatNs (ns) {
    ...
    if (ns.referClojure) {
      if (isArray(ns.referClojure.exclude)) {
        const numExcludes = arraySize(ns.referClojure.exclude)
        if (numExcludes > 0) {
