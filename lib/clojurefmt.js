...
(function (root, factory) {
  ...
  // returns an array of the closer + whitespace nodes (if any)
  // Possible (and likely) to return an empty array
  ...
  function format (inputTxt) {
    ...
    while (i < numNodes) {
      ...
      if (currentNodeIsNewline) {
        ...
        const parenTrailClosers = findCloserNodes(nodesArr, inc(i))

        if (logParenTrailClosers) {
          console.log('parenTrailClosers idx', i)
          console.log(parenTrailClosers)
          console.log('---------------------------------')
        }

        // apply the closers if applicable
        let j = 0
        const numClosers = arraySize(parenTrailClosers)
        while (j < numClosers) {
          const closerNode = parenTrailClosers[j]
