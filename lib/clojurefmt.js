  ...
  function format (inputTxt) {
    ...
    while (i < numNodes) {
      ...
      if (currentNodeIsNewline) {
        // Is there nothing but whitespace nodes and then a closer node?
        // If so, we need to squish it and put the closer on this line.
        const indentationStr = repeatString('  ', indentationLevel)
        formattedOutputString = strConcat(formattedOutputString, strConcat('\n', indentationStr))
      } else if (isString(node.text) && node.text !== '') {
        formattedOutputString = strConcat(formattedOutputString, node.text)
      }
