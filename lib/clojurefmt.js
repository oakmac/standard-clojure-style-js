...
(function (root, factory) {
  ...
  function parseNs (nodesArr) {
    ...
    while (idx < numNodes) {
      ...
      // collect :refer-macros symbols
        ...
        // console.log('whats up:', node.text)
  ...
  // FIXME: :default - if it exists - should always be the last item in this array
  ...
  function formatNs (ns) {
    ...
    if (numRequires > 0) {
      ...
      while (platformIdx < numPlatforms) {
        ...
        // Looping through the entire requires array x number of platforms
        // I think in practice the numbers involved here will be small and likely the
        // performance will be totally acceptable.
        requiresIdx = 0
        while (requiresIdx < numRequires) {
          const req = ns.requires[requiresIdx]
          const isLastRequire2 = inc(requiresIdx) === numRequires

          if (req.platform === platform) {
            outTxt = strConcat(outTxt, formatRequireLine(req, '       '))

            if (req.commentAfter && !isLastRequire2) {
              outTxt = strConcat(outTxt, ' ')
              outTxt = strConcat(outTxt, req.commentAfter)
              outTxt = strConcat(outTxt, '\n')
            } else if (isLastRequire2 && req.commentAfter && requireIsLastMainForm) {
              closeRequireParenTrail = strConcat(')) ', req.commentAfter)
              trailingParensArePrinted = true
            } else if (isLastRequire2 && req.commentAfter) {
              closeRequireParenTrail = strConcat(') ', req.commentAfter)
            } else if (isLastRequire2 && !req.commentAfter) {
              closeRequireParenTrail = ')'
            } else {
      ...
      const lastRequireHasComment = false
      const lastRequireComment = ';; comment'

      // FIXME:
      // closeRequireParenTrail can only be the following options:
