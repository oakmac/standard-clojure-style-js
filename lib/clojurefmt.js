...
(function (root, factory) {
  ...
  function numSpacesForIndentation (wrappingOpener, firstTextNodeOnNewline) {
    if (!wrappingOpener) {
      return 0
    } else {
      ...
      // console.log('firstTextNodeOnNewline:', firstTextNodeOnNewline)
  ...
  function format (inputTxt) {
    ...
    const rule3Active = false
    const rule3NumSpaces = 0
    let lineIdx = 0
    ...
    while (idx < numNodes) {
      ...
      if (currentNodeIsNewline) {
        ...
        if (inc(idx) < numNodes) {
          ...
          // is this the first newline inside of a paren stack?
          // if so, we need to check for Rule 3 indentation
          let firstLineInsidePnStck = false
          if (!rule3Active && topOfTheParenStack) {
            firstLineInsidePnStck = true
          }

          // FIXME: we need to know the colIdx of the first text node on
          // the next line for Rule 3 information
          // console.log('YYYYYYYYYY:', node)
          // console.log('HHHHHHHHHH:', nextTextNode)

          const numSpacesOnNextLine = numSpacesAfterNewline(node)
          // console.log('UUUUUUUU:', numSpacesOnNextLine)

          // colIdx of the next text node after the wrappingOpener
          // - this is available on the topOfTheParenStack object
          //
          // colIdx of the text node on this newline
          // - we need to figure this out right here
          //

          // const colIdxLine0 = topOfTheParenStack._nextWithText

          // if these are the same, then Rule3 is in effect for the rest of this
          // paren stack

          if (topOfTheParenStack) {
            console.log('parenStack first line tokens:', topOfTheParenStack._openingLineTokens)
          }

          const nextNode = nodesArr[inc(idx)] // TODO: guard against index access here?
          if (isTokenNode(nextNode)) {
            console.log('next token node:', nextNode)
          }

          const numSpaces = numSpacesForIndentation(topOfTheParenStack, nextTextNode)
