...
(function (root, factory) {
  ...
  const charRegex = /\\[\S]*/
  const keywordRegex = /:{1,2}[a-zA-Z0-9./\-_]+/
  const numberRegex = /[+-]?\d+[NM]?/
  const hexRegex = /-?0[xX][a-fA-F0-9]+[N]?/
  const radixRegex = /-?\d+[rR][a-zA-z0-9]+/
  const ratioRegex = /-?\d+\/\d+/
  const doubleBasic = /[+-]?\d+\.\d+M?/
  const doubleWithExponent = /[+-]?\d+[eE]-?\d+M?/
  const doubleWithPeriodAndExponent = /[+-]?\d+\.\d+[eE][+-]?\d+M?/

  const useNegativeLookaheadTokenParser = true

  const tokenHeadChar = '()\\[\\]{}\\"@~^;`#\\\''
  const tokenTailChars = '()\\[\\]{}\\"@^;`'

  // const testRegex1 = new RegExp('[^' + tokenHeadChar + wsNormals + '][^' + tokenTailChars + wsNormals + ']*')
  // console.assert(testRegex1.test(':smile'), 'aaa')
  // console.assert(testRegex1.test(']'), 'bbb')

  if (useNegativeLookaheadTokenParser) {
    const tokenReStr = '[^' + tokenHeadChar + wsNormals + '][^' + tokenTailChars + wsNormals + ']*'
    const zzz = '\\\\[()\\[\\]{}"@^;`]'
    parsers.token = Regex({ name: 'token', regex: new RegExp('(##)?' + '(' + zzz + '|' + tokenReStr + ')') })
  } else {
  // NOTE: order matters here
    parsers.token = Choice({
      parsers: [
        Regex({ name: 'token', regex: primitivesRegex }),
        Regex({ name: 'token', regex: ratioRegex }),
        Regex({ name: 'token', regex: radixRegex }),
        Regex({ name: 'token', regex: hexRegex }),
        Regex({ name: 'token', regex: doubleWithPeriodAndExponent }),
        Regex({ name: 'token', regex: doubleWithExponent }),
        Regex({ name: 'token', regex: doubleBasic }),
        Regex({ name: 'token', regex: numberRegex }),
        Regex({ name: 'token', regex: keywordRegex }),
        Regex({ name: 'token', regex: qualifiedSymbolRegex }),
        Regex({ name: 'token', regex: simpleSymbolRegex }),
        Regex({ name: 'token', regex: charRegex })
      ]
    })
  }
