...
(function (root, factory) {
  ...
  //       if (isOpener(childNode)) {
  //         indentationLevel = inc(indentationLevel)
  //       } else if (isCloser(childNode)) {
  ...
  function format (inputTxt) {
    ...
    let currentLineLastOpener = null
    let prevLineLastOpener = null
    let prevLineLastOpenerColIdx = 0
    let colIdx = 0
    let currentLineLastOpenerColIdx = 0
    while (idx < numNodes) {
      // let prevNode = null
      // if (i > 0) { prevNode = nodesArr[dec(i)] }
      const node = nodesArr[idx]
      // let nextNode = null
      // if (i < dec(numNodes)) { nextNode = nodesArr[inc(i)] }

      const currentNodeIsNewline = isNewlineNode(node)

      if (isOpener(node)) {
        parenNestingDepth = inc(parenNestingDepth)
        currentLineLastOpener = node
        currentLineLastOpenerColIdx = colIdx
      } else if (isCloser(node)) {
        parenNestingDepth = dec(parenNestingDepth)
      }

      const logParenTrailClosers = false
      const logLastLineOpener = false
      ...
      if (currentNodeIsNewline) {
        ...
        if (logParenTrailClosers) {
          console.log('parenTrailClosers idx:', idx)
        ...
        if (inc(idx) < numNodes) {
          // FIXME: this logic / approach is wrong
          // we need the "previous line last opener that is not closed"

          // const nnn = 2
          // if (prevLineLastOpener && prevLineLastOpener.text === '[') {
          //   console.log('set 1 for indentation', prevLineLastOpener)
          //   nnn = 1
          // }
          // const numSpaces = prevLineLastOpenerColIdx + nnn
          const numSpaces = parenNestingDepth * 2

          if (prevLineLastOpener) {
            console.log('prevLineLastOpener:', prevLineLastOpener)
            console.log('prevLineLastOpenerColIdx:', prevLineLastOpenerColIdx)
          }

          const indentationStr = repeatString(' ', numSpaces)
          // const indentationStr = repeatString('  ', parenNestingDepth)

          if (logLastLineOpener) {
            console.log('current line opener:', currentLineLastOpener)
            console.log('current line opener col idx:', currentLineLastOpenerColIdx)
            console.log('prev line opener:', prevLineLastOpener)
            console.log('prev line opener col idx:', prevLineLastOpenerColIdx)
            console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
          }
        ...
        // save this line's last opener
        prevLineLastOpener = currentLineLastOpener
        prevLineLastOpenerColIdx = currentLineLastOpenerColIdx
    ...
    // const formattedOutput = format1(inputTxt, tree)
