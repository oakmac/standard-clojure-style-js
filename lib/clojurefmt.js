...
(function (root, factory) {
  ...
  // returns the idx + node if found, null otherwise
  ...
  function parseNs (nodesArr) {
    ...
    while (idx < numNodes) {
      ...
      if (idx > nsNodeIdx && nextTextNodeIsNsSymbol && isTokenNode(node) && nodeContainsText(node)) {
        result.nsSymbol = node.text
        nextTextNodeIsNsSymbol = false
        // collect the require :as
      } else if (idx > requireNodeIdx && insideRequireForm && nextTokenIsAsSymbol && isTokenNode(node) && nodeContainsText(node)) {
        nextTokenIsAsSymbol = false
        result.requires[activeRequireIdx].as = node.text
      } else if (idx > requireNodeIdx && insideRequireForm && isTokenNode(node) && isRequireAsNode(node)) {
        nextTokenIsAsSymbol = true
        // collect require imports
      } else if (idx > requireNodeIdx && insideRequireForm && isTokenNode(node) && nodeContainsText(node)) {
  ...
  function format (inputTxt) {
    ...
    while (idx < numNodes) {
      ...
      const nextTextNodeResult = findNextNodeWithText(nodesArr, inc(idx))
      let nextTextNode = null
      // let nextTextNodeIdx = null
      if (nextTextNodeResult) {
        nextTextNode = nextTextNodeResult.node
        // nextTextNodeIdx = nextTextNodeResult.idx
      }
      ...
      if (currentNodeIsNewline) {
        ...
        //   skipPrintingThisNode = true
        // } else if (isWhitespaceNode(node) && !outputTxtContainsChars) {
        //   skipPrintingThisNode = true
        // }
        ...
        if (!skipPrintingThisNode) {
          ...
          if (isParenCloser(node) && insideNsForm && parenNestingDepth === 0) {
            insideNsForm = false
            nsEndStringIdx = strLen(outTxt)
          }
        }
