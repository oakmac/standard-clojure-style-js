  ...
  function format (inputTxt) {
    ...
    while (idx < numNodes) {
      ...
      if (currentNodeIsNewline) {
        ...
        const addSpaceAfterThisNode =
          (isTokenNode(node) && nextTextNode && isParenOpener(nextTextNode)) ||
          (isParenCloser(node) && nextTextNode && (isTokenNode(nextTextNode) || isParenOpener(nextTextNode)))
