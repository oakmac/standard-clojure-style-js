...
(function (root, factory) {
  ...
  // FIXME: rename this function to be more generic
  // returns the next token from nodes that is not the following:
  // - keyword
  // - metadata
  // - closing paren
  // - comment
  ...
  function parseNs (nodesArr) {
    ...
    let insideSingleRequireList = false
    ...
    let requireNodeParenNestingDepth = -1
    ...
    while (idx < numNodes) {
      ...
      if (parenNestingDepth === 1 && isNsNode(node)) {
        ...
        requireNodeIdx = idx
        requireNodeParenNestingDepth = parenNestingDepth
      ...
      } else if (insideRequireForm && !insideSingleRequireList && parenNestingDepth >= 3 && requireSymbolIdx > 0) {
        insideSingleRequireList = true
      }
      ...
      // FIXME: the parenNestingDepth comparison here will break when we add Reader Conditionals
      } else if (insideRequireForm && idx > requireNodeIdx && parenNestingDepth === requireNodeParenNestingDepth && isTokenNode2 && isTextNode && requireSymbolIdx === -1 && !isKeywordNode(node)) {
      ...
      // collect symbols inside of a prefix list
        ...
        const requireTxt = strConcat3(prefixListPrefix, '.', node.text)
        ...
        // FIXME: we should only attach the comments to the first require here
      ...
      // collect :require symbol
      } else if (insideRequireForm && idx > requireNodeIdx && isTokenNode2 && isTextNode && requireSymbolIdx === -1 && !isKeywordNode(node)) {
        if (!isArray(result.requires)) {
          result.requires = []
        }
        ...
        // FIXME: rename this searching function to be more generic
