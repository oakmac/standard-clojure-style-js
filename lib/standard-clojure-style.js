...
function isThanksgivingNode (n) {
  if (n && isNewlineNode(n)) {
    const split = strSplit(n.text, '\n')
    if (split.length === 2) {
      if (split[1].endsWith(',')) { // FIXME: this is incomplete, there could be ",  "
        return true
      }
    }
  ...
  return false
  // return n && isWhitespaceNode(n) && isNewlineNode(n)
}
  ...
  // FIXME: this is where the bug is
  // we should not look past a "\n      ," whitespace node
  function findForwardClosingParens (nodes, idx) {
    ...
    while (keepSearching) {
      ...
      if (!node) {
        keepSearching = false
      } else if (isThanksgivingNode(node)) {
keepSearching = false
    ...
    while (idx < numNodes) {
      ...
      if (insideTheIgnoreZone) {
        ...
        if (currentNodeIsWhitespace && !currentNodeIsNewline && nextTextNode && isCommentNode(nextTextNode)) {
          ...
          // NOTE: replaceAll might not exist in all JS environments
        ...
        if (parenStackSize > 0 && !insideNsForm) {
          ...
          if (lookForwardToSlurpNodes) {
            ...
            // FIXME: this is the bug here
            // the "\n     ," node should "halt" this function from finding
            //console.log('paren trail closers:', parenTrailClosers)
            ...
            if (lineTxtHasBeenRightTrimmed) {
              // FIXME: this is the cause of the re-printed comma
        ...
        if ((nodeContainsText(node) || currentNodeIsTag) && !skipPrintingThisNode) {
          ...
          if (!skipPrintingThisNode) {
            ...
            // FIXME: the comment node ";; foo" is adding the comma here
            lineTxt = strConcat(lineTxt, nodeTxt)

            // console.log('xxx' + nodeTxt + 'xxx')
            // console.log('yyy' + lineTxt + 'yyy')
            // console.log('+++++++++++++++++++++++++++++++++++++++')
