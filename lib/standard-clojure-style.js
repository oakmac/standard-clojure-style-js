...
(function (root, factory) {
  ...
  function formatListOfRenames (itms) {
    let s = ''
  ...
  // function formatReferClojureSingleLine (key, itms, indentationStr) {
  //   const symbols = arrayPluck(itms, 'symbol')
  //   let s = strConcat(indentationStr, '(:refer-clojure ')
  //   s = strConcat3(s, key, ' [')
  //   s = strConcat(s, strJoin(symbols, ' '))
  //   s = strConcat(s, '])')
  ...
  function formatReferClojure (referClojure) {
    ...
    // there is only :exclude
      ...
      const platforms = getPlatformsFromArray(referClojure.exclude)
      const numPlatforms = arraySize(platforms)

      if (numPlatforms === 0) {
        const symbols = arrayPluck(referClojure.exclude, 'symbol')
        let s = '\n  (:refer-clojure '
        s = strConcat(s, formatKeywordFollowedByListOfSymbols(':exclude', symbols))
        return s
        ...
        return 'FIXME: write me refer-clojure :exclude multiple reader conditional platforms'
      }

    // there is only :only
    } else if (numKeys === 1 && keys[0] === ':only') {
      const platforms = getPlatformsFromArray(referClojure.only)
      const numPlatforms = arraySize(platforms)

      if (numPlatforms === 0) {
        const symbols = arrayPluck(referClojure.only, 'symbol')
        let s = '\n  (:refer-clojure '
        s = strConcat(s, formatKeywordFollowedByListOfSymbols(':only', symbols))
        return s
      } else if (numPlatforms === 1) {
        const symbols = arrayPluck(referClojure.only, 'symbol')
        let s = strConcat3('\n  #?(', platforms[0], '\n')
        ...
        return 'FIXME: write me refer-clojure :only with multiple reader conditional platforms'
      }
    ...
    // there is only :rename
      ...
      if (numPlatforms === 0) {
        ...
        s = strConcat(s, formatListOfRenames(referClojure.rename))
        s = strConcat(s, '})')
        return s
      } else if (numPlatforms === 1) {
        let s = strConcat3('\n  #?(', platforms[0], '\n')
    ...
    // there are multiple keys, put each one on it's own line
      ...
      return 'FIXME: refer-clojure with multiple keys'
    }
  }
  ...
  function formatNs (ns) {
    ...
    //     outTxt = strConcat(outTxt, '\n  (:refer-clojure :only [')
    //     outTxt = strConcat(outTxt, strJoin(onlySymbols, ' '))
    //     outTxt = strConcat(outTxt, '])')
    //   }
    ...
    //     while (idx2 < numRenames) {
    //       const isLastRename = dec(idx2) === numRequires
    //       const renameObj = ns.referClojure.rename[idx2]

    //       outTxt = strConcat3(outTxt, renameObj.fromSymbol, ' ')
    //       outTxt = strConcat(outTxt, renameObj.toSymbol)
