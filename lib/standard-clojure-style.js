...
(function (root, factory) {
  ...
  // 1) it flattens the nested tree of nodes into a single Array of nodes
  // 2) it attaches open / closing paren information to nodes
  function flattenTree (tree) {
    ...
    const pushNodeToNodes = (node) => {
      ...
      if (isParenOpener(node)) {
        ...
        const parenGroupId = createId()
        const parenGroup = {
          expectedClosingParenText: 'FIXME: write this',
          openParenNodeId: node.id,
          openParenText: node.text,
          parenGroupId: parenGroupId,
          parenNestingDepth: parenNestingDepth
        }
        stackPush(parenStack, parenGroup)

        node.parenGroupId = parenGroupId
        node.parenNestingDepth = parenNestingDepth
        node.expectedClosingParenText = 'FIXME: write this'
      } else if (isParenCloser(node)) {

        const topOfTheParenStack = stackPop(parenStack)
        if (!topOfTheParenStack) {
          throw new Error('FIXME: unbalance paren stack')
        }

        node.parenGroupId = topOfTheParenStack.parenGroupId
        node.parenNestingDepth = topOfTheParenStack.parenNestingDepth
        node.openParenNodeId = topOfTheParenStack.openParenNodeId
