...
(function (root, factory) {
  ...
  function parseNs (nodesArr) {
    ...
    while (continueParsingNsForm) {
      ...
      const nonBlankText4 = isNodeWithNonBlankText(node)

      if (parenNestingDepth >= 1 && isTokenNode2 && nonBlankText4) {
        tokenListIdx55555555 = inc(tokenListIdx55555555)
        // console.log('idx:', tokenListIdx55555555, 'zzz', node)
      }

//       const nodeIsFirstSymbolInsideList =       tokenListIdx55555555 === 1

// // isNodeWithNonBlankText
//       if (nodeIsFirstSymbolInsideList) {
//         // console.log('xxx' + node.text + 'xxx')
//       }

      if (parenNestingDepth === 1 && isNsNode(node) && tokenListIdx55555555 === 1) {
    ...
    if (result.nsSymbol === null) {
      result.nsFormNotFound = true
    }
  ...
  function formatNodes (nodesArr, parsedNs) {
    ...
    while (idx < numNodes) {
      ...
      if (insideTheIgnoreZone) {
        ...
        // ttttttttttttt
        if (nsStartStringIdx === -1 && parenNestingDepth === 1 && parsedNs.nsSymbol !== null && isNsNode(node)) {
          insideNsForm = true
