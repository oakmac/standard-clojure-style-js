...
(function (root, factory) {
  ...
  function findIgnoreNodeIdx (allNodes, startIdx, ignoreNodeId) {
    ...
    while (idx < maxIdx) {
      ...
      } else if (isString(node.text) && node.text !== '' && !isWhitespaceNode(node)) {
        return idx
      }
  ...
  function formatNodes (nodesArr, parsedNs) {
    ...
    while (idx < numNodes) {
      ...
      // FIXME: this could be extracted out into a function
      // FIXME: this needs to handle the #_ {:standard-clj/ignore true} map scenario
      if (isDiscardNode(node) && nextTextNode) {
        ...
        if (isWhitespaceNode(nextTextNode) && nextNextIdx < numNodes) {
          ...
          if (isStandardCljIgnoreKeyword(nextNextTextNode)) {
            const ignoreStartingIdx = findIgnoreNodeIdx(nodesArr, inc(idx), nextNextTextNode.id)
            console.log('1 ignore starting at idx:', nextNextIdx)
            console.log('HHHHH:', ignoreStartingIdx)
            console.log('aaaaa:', nodesArr[ignoreStartingIdx])
            // FIXME: nodesArr[ignoreStartingIdx] is probably a paren opener
          }
        } else if (isStandardCljIgnoreKeyword(nextTextNode)) {
          const ignoreStartingIdx = findIgnoreNodeIdx(nodesArr, inc(idx), nextTextNode.id)
          console.log('2 ignore starting at idx:', inc(idx))
          console.log('TTTTT:', ignoreStartingIdx)
          console.log('bbbbb:', nodesArr[ignoreStartingIdx])
          // FIXME: nodesArr[ignoreStartingIdx] is probably a paren opener
        }
      }
