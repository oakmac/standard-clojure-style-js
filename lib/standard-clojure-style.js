...
(function (root, factory) {
  ...
  function parseNs (nodesArr) {
    ...
    const ps = {}
    ps.insideNsForm = false
    ps.genClassNodeIdx = -1
    ps.continueParsingNsForm = true
    ps.nsFormEndsLineIdx = -1
    ps.parenNestingDepth = 0
    ...
    while (ps.continueParsingNsForm) {
      const node = nodesArr[idx]
      ...
      if (ps.parenNestingDepth >= 1 && isTokenNode2 && nodeHasNonBlankText) {
        numSymbolsInsideList = inc(numSymbolsInsideList)
      }

      if (ps.parenNestingDepth === 1 && isNsNode(node) && numSymbolsInsideList === 1) {
        ps.insideNsForm = true
        nextTextNodeIsNsSymbol = true
        nsNodeIdx = idx
      } else if (ps.insideNsForm && isReferClojureNode(node)) {
        insideReferClojureForm = true
        referClojureParenNestingDepth = ps.parenNestingDepth
        sectionToAttachEolCommentsTo = 'refer-clojure'
        referClojureNodeIdx = idx
        beyondNsMetadata = true
      } else if (ps.insideNsForm && isRequireNode(node)) {
        insideRequireForm = true
        requireFormParenNestingDepth = ps.parenNestingDepth
        requireFormLineNo = lineNo
        requireNodeIdx = idx
        beyondNsMetadata = true
        sectionToAttachEolCommentsTo = 'require'
      } else if (ps.insideNsForm && isImportNode(node)) {
        insideImportForm = true
        importFormLineNo = lineNo
        importNodeIdx = idx
        importNodeParenNestingDepth = ps.parenNestingDepth
        beyondNsMetadata = true
        sectionToAttachEolCommentsTo = 'import'
      } else if (ps.insideNsForm && isRequireMacrosKeyword(node)) {
        insideRequireMacrosForm = true
        requireMacrosNodeIdx = idx
        requireMacrosLineNo = lineNo
        requireMacrosParenNestingDepth = ps.parenNestingDepth
        beyondNsMetadata = true
        sectionToAttachEolCommentsTo = 'require-macros'
      } else if (ps.insideNsForm && isGenClassNode(node)) {
        insideGenClass = true
        ps.genClassNodeIdx = idx
        beyondNsMetadata = true
        sectionToAttachEolCommentsTo = 'gen-class'
      }

      if (isParenOpener(node)) {
        ps.parenNestingDepth = inc(ps.parenNestingDepth)
        stackPush(parenStack, node)

        numSymbolsInsideList = 0

        if (ps.insideNsForm && isReaderConditionalOpener(node)) {
          insideReaderConditional = true
          currentReaderConditionalPlatform = null
          readerConditionalParenNestingDepth = ps.parenNestingDepth
        } else if (insideRequireForm && requireListParenNestingDepth === -1) {
          insideRequireList = true
          requireListParenNestingDepth = ps.parenNestingDepth
        } else if (insideImportForm && ps.parenNestingDepth > importNodeParenNestingDepth) {
          insideImportPackageList = true
        }
      } else if (isParenCloser(node)) {
        ps.parenNestingDepth = dec(ps.parenNestingDepth)
        stackPop(parenStack)

        // We can assume there is only one ns form per file and exit the main
        // loop once we have finished parsing it.
        if (ps.insideNsForm && ps.parenNestingDepth === 0) {
          ps.insideNsForm = false
          ps.nsFormEndsLineIdx = lineNo
        }
        ...
        if (insideRequireForm && ps.parenNestingDepth < requireFormParenNestingDepth) {
          insideRequireForm = false
          requireFormParenNestingDepth = -1
        }
        if (insideReferClojureForm && ps.parenNestingDepth < referClojureParenNestingDepth) {
        ...
        if (referIdx > 0 && ps.parenNestingDepth <= referParenNestingDepth) {
          referIdx = -1
          referParenNestingDepth = -1
        }

        // we are finished collecting :rename symbols
        if (renameIdx > 0 && ps.parenNestingDepth <= renameParenNestingDepth) {
          renameIdx = -1
          renameParenNestingDepth = -1
        }

        // we are finished collecting require prefix list symbols
        if (insideRequireList && ps.parenNestingDepth < requireListParenNestingDepth) {
        ...
        if (insideReaderConditional && ps.parenNestingDepth === dec(readerConditionalParenNestingDepth)) {
          insideReaderConditional = false
          currentReaderConditionalPlatform = null
          readerConditionalParenNestingDepth = -1
        }
        if (idx > referMacrosIdx && ps.parenNestingDepth <= referMacrosParenNestingDepth) {
          referMacrosIdx = -1
          referMacrosParenNestingDepth = -1
        }
        if (insideImportForm && ps.parenNestingDepth < importNodeParenNestingDepth) {
          insideImportForm = false
          importNodeIdx = -1
          importNodeParenNestingDepth = -1
        }
        if (insideRequireMacrosForm && ps.parenNestingDepth < requireMacrosParenNestingDepth) {
          insideRequireMacrosForm = false
          requireMacrosParenNestingDepth = -1
          requireMacrosNodeIdx = -1
          requireMacrosAsNodeIdx = -1
        }
        if (collectRequireExcludeSymbols && ps.parenNestingDepth < requireExcludeSymbolParenDepth) {
      ...
      } else if (!insideNsMetadataHashMap && !insideNsMetadataShorthand && ps.insideNsForm && nsSymbolIdx < 0 && node.name === 'meta') {
      ...
      } else if (ps.insideNsForm && idx > nsNodeIdx && ps.parenNestingDepth >= 1 && !beyondNsMetadata && !insideReaderComment && !insideNsMetadataShorthand && !insideNsMetadataHashMap && node.name === '.open' && node.text === '{') {
        insideNsMetadataHashMap = true
        nextTextNodeIsMetadataKey = true
      ...
      } else if (insideReaderConditional && ps.parenNestingDepth === readerConditionalParenNestingDepth && isKeywordNode(node)) {
        currentReaderConditionalPlatform = node.text

      // collect single-line comments
      } else if (ps.insideNsForm && idx > nsNodeIdx && prevNodeIsNewline && isCommentNode2) {
        stackPush(singleLineComments, node.text)

      // collect reader macro comment line(s)
      } else if (ps.insideNsForm && idx > nsNodeIdx && prevNodeIsNewline && isReaderCommentNode2) {
      ...
      // collect comments at the end of a line
        ...
        if (!ps.insideNsForm && lineNo === lineOfLastCommentRecording) {
      ...
      } else if (ps.insideNsForm && idx > nsNodeIdx && ps.parenNestingDepth === 1 && !beyondNsMetadata && !insideNsMetadataShorthand && !insideNsMetadataHashMap && isStringNode(node)) {
      ...
      } else if (idx > inc(referClojureNodeIdx) && collectReferClojureExcludeSymbols && ps.parenNestingDepth >= 3 && isTokenNode2 && isTextNode && result.referClojure && isArray(result.referClojure.exclude)) {
        const symbolObj = {}
      ...
      } else if (idx > inc(referClojureNodeIdx) && collectReferClojureOnlySymbols && ps.parenNestingDepth >= 3 && isTokenNode2 && isTextNode && result.referClojure && isArray(result.referClojure.only)) {
        const symbolObj = {
          symbol: node.text
        }
      ...
      } else if (idx > inc(referClojureNodeIdx) && collectReferClojureRenameSymbols && ps.parenNestingDepth >= 3 && isTokenNode2 && isTextNode && result.referClojure && isArray(result.referClojure.rename)) {
      ...
      // is this :refer-macros ?
        ...
        referMacrosIdx = idx
        referMacrosParenNestingDepth = ps.parenNestingDepth

      // collect :refer-macros symbols
      } else if (idx > referMacrosIdx && insideRequireForm && ps.parenNestingDepth === inc(referMacrosParenNestingDepth) && isTokenNode2 && isTextNode) {
      ...
      // is this :require :refer ?
        ...
        referIdx = idx
        referParenNestingDepth = ps.parenNestingDepth
      ...
      } else if (idx > requireNodeIdx && insideRequireForm && isTokenNode2 && collectRequireExcludeSymbols && ps.parenNestingDepth > requireExcludeSymbolParenDepth) {
        const symbolObj = {
          symbol: node.text
        }
      ...
      // is this :require :exclude ?
        ...
        requireExcludeSymbolParenDepth = ps.parenNestingDepth
      ...
      // :rename keyword inside a :require list
        ...
        renameIdx = idx
        renameParenNestingDepth = ps.parenNestingDepth
        renamesTmp = []

      // collect :require renames
      } else if (insideRequireForm && insideRequireList && renameIdx > 0 && idx > renameIdx && ps.parenNestingDepth > renameParenNestingDepth && isTokenNode2 && isTextNode) {
        stackPush(renamesTmp, node.text)
      ...
      } else if (idx > referIdx && insideRequireForm && referParenNestingDepth !== -1 && ps.parenNestingDepth > referParenNestingDepth && isTokenNode2 && isTextNode) {
      ...
      } else if (insideGenClass && idx === ps.genClassNodeIdx) {
        result.genClass = {}
      ...
      } else if (insideGenClass && idx > ps.genClassNodeIdx && isTextNode && genClassToggle === 0 && isGenClassKeyword(node)) {
      ...
      } else if (insideGenClass && idx > ps.genClassNodeIdx && genClassToggle === 1 && genClassKeyStr === 'prefix' && isStringNode(node)) {
        result.genClass.prefix.value = strConcat3('"', getTextFromStringNode(node), '"')
        genClassToggle = 0
        genClassValueLineNo = lineNo

      // other :gen-class values
      } else if (insideGenClass && idx > ps.genClassNodeIdx && isTextNode && isTokenNode2 && genClassToggle === 1) {
      ...
      } else if (ps.insideNsForm && isTokenNode2 && ps.parenNestingDepth >= 1 && isUseNode(node)) {
      ...
      if (idx >= numNodes) {
        ps.continueParsingNsForm = false
      // exit if we have finished parsing the ns form
      } else if (nsNodeIdx > 0 && !ps.insideNsForm && lineNo >= inc(inc(ps.nsFormEndsLineIdx))) {
        ps.continueParsingNsForm = false
      }
