...
(function (root, factory) {
  ...
  // Parses the body of a String character-by-character
  // NOTE: this function exists for porting to other languages that do not support
  // the Regex version of the String parser
  function stringBodyParser (txt, pos) {
    const maxLength = strLen(txt)
    ...
    while (keepSearching) {
      const ch = charAt(txt, charIdx)
      if (ch === null || ch === '') {
        keepSearching = false
      } else if (ch === '\\') {
        const nextChar = charAt(txt, inc(charIdx))
        if (isString(nextChar) && nextChar !== '') {
  ...
  const whitespaceCommonChars = [' ', ',', '\n', '\r', '\t', '\f']
  const whitespaceUnicodeChars = ['\u000B', '\u001C', '\u001D', '\u001E', '\u001F', '\u2028', '\u2029', '\u1680', '\u2000', '\u2001', '\u2002', '\u2003', '\u2004', '\u2005', '\u2006', '\u2008', '\u2009', '\u200a', '\u205f', '\u3000']
  const whitespaceChars2 = whitespaceCommonChars.concat(whitespaceUnicodeChars)

  const tokenHeadChars2 = ['(', ')', '[', ']', '{', '}', '"', '@', '~', '^', ';', '`', '#', "'"]
  const tokenTailChars2 = ['(', ')', '[', ']', '{', '}', '"', '@', '^', ';', '`']

  const tokenHeadChars3 = tokenHeadChars2.concat(whitespaceChars2)
  const tokenTailChars3 = tokenTailChars2.concat(whitespaceChars2)

  function isValidTokenHeadChar (ch) {
    return !arrayContainsItm(tokenHeadChars3, ch)
  }

  function isValidTokenTailChar (ch) {
    return !arrayContainsItm(tokenTailChars3, ch)
  }

  // parses a Token character-by-character
  // NOTE: this function exists for porting to other languages that do not support
  // the Regex version of the Token parser
  function tokenParser (txt, pos) {
    const maxLength = strLen(txt)
    ...
    const firstTwoChars = substr(txt, pos, inc(inc(pos)))
    if (firstTwoChars === '##') {
      parsedTxt = '##'
  ...
  function specialCharParser (txt, pos) {
    const maxLength = strLen(txt)
    if (maxLength === 0) return null

    const firstChar = charAt(txt, pos)
    if (firstChar === '\\') {
      const secondChar = charAt(txt, inc(pos))
      if (isString(secondChar) && arrayContainsItm(specialChars, secondChar)) {
        return Node({
          endIdx: inc(inc(pos)),
          name: 'token',
          startIdx: pos,
  ...
  const useRegexTokenParser = false

  if (useRegexTokenParser) {
