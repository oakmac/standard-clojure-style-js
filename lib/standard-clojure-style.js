...
(function (root, factory) {
  ...
  function isParenOpener (n) {
    return n.text === '(' || n.text === '[' || n.text === '{' || n.text === '#{' || n.text === '#(' || n.text === '#?(' || n.text === '#?@('
  }
  ...
  // given a root node, returns a string of all the text found within it's children
  ...
  // returns an array of column indexes where nodes start for a single line
  function collectNodeIndexes (nodes, idx) {
    // HACK HACK HACK
    // We should change the parser to separate newline nodes from
    // whitespace nodes.
    // https://github.com/oakmac/standard-clojure-style-js/issues/15

    // the first node should be a newline node
    const firstNode = nodes[idx]
    const initialSpaces = dec(strLen(firstNode.text)) // TODO: write function to get number of spaces from whitespace node
    ...
    while (keepSearching) {
      ...
      if (!node) {
        ...
        if (isString(nodeTxt)) {
          const nodeLength = strLen(nodeTxt)

          if (!isWhitespaceNode(node)) {
            stackPush(colIndexes, colIdx)
          }

          colIdx = colIdx + nodeLength
        }
      }
  ...
  function parseNs (nodesArr) {
    ...
    while (idx < numNodes) {
      ...
      } else if (idx > nsNodeIdx && prevNodeIsNewline && isCommentNode2) {
        stackPush(singleLineComments, node.text)

      // collect reader macro comment line(s)
      } else if (idx > nsNodeIdx && prevNodeIsNewline && isReaderCommentNode2) {
      ...
      } else if (idx > nsNodeIdx && parenNestingDepth >= 1 && referClojureNodeIdx < 0 && requireNodeIdx < 0 && importNodeIdx < 0 && !insideGenClass && isStringNode(node)) {
  ...
  function formatRequireLine (req, initialIndentation) {
    ...
    if (isArray(req.refer) && arraySize(req.refer) > 0) {
      ...
      // FIXME: change the parser so that this is always an array of Objects, not an array of Strings
      let idx = 0
      const numReferSymbols = arraySize(req.refer)
      while (idx < numReferSymbols) {
        const itm = req.refer[idx]
        if (isString(itm)) {
          outTxt = strConcat(outTxt, itm)
        } else {
          outTxt = strConcat(outTxt, itm.symbol)
        }

        if (inc(idx) !== numReferSymbols) {
  ...
  function formatReferClojure (referClojure) {
    ...
    // there is only :rename
      ...
      if (numPlatforms === 0) {
        ...
        return 'FIXME: refer-clojure, :rename, multiple reader conditional platforms'
      }
  ...
  function format (inputTxt) {
    ...
    let crazyCrazyIdx = -1 // this is the index of the nodes for "comment after form" alignment
    // FIXME: we seriously need a better name for this

    const parenStack = []
    let nodeColumnIndexesAfterAdjustment = []
    let nodeColumnIndexesBeforeAdjustment = []
    let upcomingLineOriginalIndexes = null
    ...
    while (idx < numNodes) {
      ...
      if (isParenOpener(node)) {
        ...
        nodeWithExtraInfo._openingLineNodes = []
      ...
      if (currentNodeIsNewline) {
        ...
        // collect the column indexes of nodes on the next line before we modify anything
        upcomingLineOriginalIndexes = collectNodeIndexes(nodesArr, idx)
        // console.log('original column indexes:', originalColumnIndexes)
        ...
        if (nextLineContainsOnlyOneComment) {
          // const nextLineCommentNode = nodesArr[inc(idx)]

          // TODO: there is probably a more robust way of determining this
          nextLineCommentColIdx = dec(strLen(node.text))

          // console.log('current node:', node)
          // console.log('next line is only comment line:', nodesArr[inc(idx)])
          // console.log('previous line node indexes:', nodeColumnIndexesAfterAdjustment)
        }

        // console.log('idx:', idx, 'zzz' + node.text + 'zzz')
        // console.log(nextLineContainsOnlyOneComment)
        ...
        // print the newline and the next line's indentation level
        ...
        // if (outputTxtContainsChars) {
          ...
          if (topOfTheParenStack && topOfTheParenStack._parenOpenerLineIdx === lineIdx) {
            let keepSearchingForAlignmentNode = true
            ...
            if (numOpeningLineNodes > 2) {
              while (keepSearchingForAlignmentNode) {
                ...
                if (openingLineNode) {
                  ...
                  if (pastFirstWhitespaceNode === true && isNodeWithNonBlankText(openingLineNode) && openingLineNode._colIdx === numSpacesOnNextLine) {
                    // Rule 3 is activated 👍
                    ...
                    keepSearchingForAlignmentNode = false
                  } else if (pastFirstWhitespaceNode === false && isWhitespaceNode(openingLineNode)) {
          ...
          if (nextLineContainsOnlyOneComment) {
            let idx4 = 0
            const numPrevLineNodeIndexes = arraySize(nodeColumnIndexesBeforeAdjustment)
            while (idx4 < numPrevLineNodeIndexes) {
              if (nextLineCommentColIdx === nodeColumnIndexesBeforeAdjustment[idx4]) {
                commentLooksAlignedWithPreviousForm = true
                crazyCrazyIdx = idx4

                // we can exit this loop now
                idx4 = inc(numPrevLineNodeIndexes)
              }
          ...
          // } else if (nextLineContainsOnlyOneComment) {
            // numSpaces = 37

            // console.log('line idx:', lineIdx)
            // console.log('original indexes:', nodeColumnIndexesBeforeAdjustment)
            // console.log('adjusted indexes:', nodeColumnIndexesAfterAdjustment)

            numSpaces = nodeColumnIndexesAfterAdjustment[crazyCrazyIdx]

            // FIXME: now this needs to be adjusted to line up with the ADJUSTED node on
            // the line above
            // not the original colIdx
            // numSpaces = nextLineCommentColIdx
          } else {
          ...
          // reset the previous line node indexes
          nodeColumnIndexesAfterAdjustment = []
          nodeColumnIndexesBeforeAdjustment = upcomingLineOriginalIndexes
        ...
        // add the colIdx of this node to the list of indexes
        // NOTE: this is the index AFTER our adjustments
        if (!currentNodeIsWhitespace) {
          stackPush(nodeColumnIndexesAfterAdjustment, colIdx)
        }
