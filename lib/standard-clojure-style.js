...
(function (root, factory) {
  ...
  function parseNs (nodesArr) {
    ...
    while (continueParsingNsForm) {
      ...
      // :gen-class :name value with metadata
      } else if (insideGenClass && idx > genClassNodeIdx && genClassToggle === 1 && genClassKeyStr === 'name' && node.name === 'meta') {
        const nameForm = splitGenClassMetadataForm(node)
        const name = getGenClassSymbolText(nameForm.bodyNode)
        if (!name) {
          throw new Error(':gen-class :name must be a symbol or string.')
        }

        if (isStringWithChars(nameForm.metadata)) {
          result.genClass.name.metadata = nameForm.metadata
        }
        result.genClass.name.value = name
        genClassToggle = 0
