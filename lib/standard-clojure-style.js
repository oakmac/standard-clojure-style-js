...
(function (root, factory) {
  ...
  function format (inputTxt) {
    ...
    while (idx < numNodes) {
      ...
      if (isParenOpener(node)) {
        // we potentially need to add this opener node to the current openingLineTokens
        const topOfTheParenStack777 = stackPeek(parenStack, 0)
        if (topOfTheParenStack777) {
          const onOpeningLineOfParenStack999999 = lineIdx === topOfTheParenStack777._parenOpenerLineIdx
          if (onOpeningLineOfParenStack999999) {
            node._colIdx = colIdx
            node._lineIdx = lineIdx

            // FIXME: make language neutral
            topOfTheParenStack777._openingLineTokens.push(node)
          // stackPush(topOfTheParenStack._openingLineTokens, node)
          }
        }
      ...
      // FIXME: bug here
      // This is only looking for token nodes. We need to look for other nodes types as well.
      // Specifically, we need to look for open parens here. When we see an open paren in the block
      // above this code, we change the parenStack. But we need to put this opener on the current parenStack
      ...
      if (currentNodeIsNewline) {
        ...
        if (outputTxtContainsChars && !isLastNode) {
          ...
          if (topOfTheParenStack && topOfTheParenStack._parenOpenerLineIdx === lineIdx) {
            ...
            // console.log('top of the paren stack:', topOfTheParenStack)
            ...
            while (openingLineTokensIdx < numOpeningLineTokens) {
              ...
              // console.log('opening line token:', openingLineToken)

              if (openingLineToken._colIdx === numSpacesOnNextLine) {
                // console.log('RULE 3 ACTIVE RULE 3 ACTIVE RULE 3 ACTIVE')
