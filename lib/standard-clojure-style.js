...
(function (root, factory) {
  ...
  // Are all of the nodes on the next line already slurped up?
  ...
  function formatNodes (nodesArr, parsedNs) {
    ...
    while (idx < numNodes) {
      ...
      // FIXME: rename these
      const isCommentFollowedByNewlineInsideParenStack = isCommentNode(node) && nextTextNode && isNewlineNode(nextTextNode) && arraySize(parenStack) !== 0
      const isNewlineInsideParenStack = isNewlineNode(node) && arraySize(parenStack) !== 0
      if (isCommentFollowedByNewlineInsideParenStack || isNewlineInsideParenStack) {
