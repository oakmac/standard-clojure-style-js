...
(function (root, factory) {
  ...
  function compareSymbols (requireA, requireB) {
  ...
  function formatNs (ns) {
    ...
    if (ns.referClojure) {
      ...
      if (isArray(ns.referClojure.only) && arraySize(ns.referClojure.only) > 0) {
        ...
        let idx = 0
        const numReferClojureOnlySymbols = arraySize(ns.referClojure.only)
        while (idx < numReferClojureOnlySymbols) {
          outTxt = strConcat(outTxt, ns.referClojure.only[idx].symbol)
          if (idx < dec(numReferClojureOnlySymbols)) {
            outTxt = strConcat(outTxt, ' ')
          }
          idx = inc(idx)
        }
