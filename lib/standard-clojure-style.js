  ...
  function collectNodeIndexes (nodes, idx) {
    ...
    while (keepSearching) {
      ...
      if (!node) {
        ...
        // FIXME: pretty sure we should not be counting closing parens here
        const nodeTxt = node.text
        if (isString(nodeTxt)) {
  ...
  function formatNodes (nodesArr, parsedNs) {
    ...
    while (idx < numNodes) {
      ...
      if (currentNodeIsNewline) {
        ...
        // FIXME: I am pretty sure the upcomingLineOriginalIndexes array could be replaced with
        // checks to _origColIdx
        // And we could remove the collectNodeIndexes function here. Would require a refactor.
        ...
        if (!skipPrintingThisNode) {
          ...
          lineTxt = strConcat(lineTxt, node.text)

          if (lineTxt !== '') {
            outputTxtContainsChars = true
          }

          // add the printed colIdx of this node to the node
