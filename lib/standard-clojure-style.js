...
(function (root, factory) {
  ...
  function isTaggedLiteralOpener (opener) {
    return opener.text === '#js'
  }
  ...
  function numSpacesForIndentation (wrappingOpener) {
    if (!wrappingOpener) {
      ...
      // console.log('xxxxx:', wrappingOpener)

      // if (isReaderConditionalOpener(wrappingOpener) || isTaggedLiteralOpener(wrappingOpener)) {
      ...
      if (topOfTheParenStack && nodeContainsText(node)) {
        ...
        if (onOpeningLineOfParenStack) {
          ...
          if (taggedNodeIdx === dec(idx)) {
            // console.log('tagged literal node:', node)
            ...
            if (numOpeningLineNodes > 2) {
              while (searchForAlignmentNode) {
                ...
                if (openingLineNode) {
                  ...
                  console.log('opening line node:', openingLineNode)

                  if (openingLineNode._nodeIsTagLiteral) {
                    console.log(openingLineNode)
                    console.log("zzzz", numSpacesOnNextLine)

                    // openingLineNode._origColIdx = dec(openingLineNode._origColIdx)
                    // openingLineNode._printedColIdx = openingLineNode._printedColIdx + 25
                  }
                  ...
                  if (pastFirstWhitespaceNode && isNodeWithNonBlankText(openingLineNode) && openingLineNode._origColIdx === numSpacesOnNextLine) {
                    ...
                    // console.log(topOfTheParenStack._openingLineNodes)
                    // console.log('+++++++++++++++++++++++++++++++++++++++++++++++++')
...
//                     if (topOfTheParenStack._nodeIsTagLiteral) {
// topOfTheParenStack._rule3NumSpaces = dec(dec(dec(openingLineNode._printedColIdx)))
//                     } else {

                    topOfTheParenStack._rule3NumSpaces = openingLineNode._printedColIdx
                    // }

                    // this is confusing: the wrapper node for a tagged literal is the node before
