...
(function (root, factory) {
  ...
  function nsCanCollectRequireSymbolNotInsideListOrVector (ps) {
    return ps.insideRequireForm && !ps.insideRequireList && ps.idx > ps.requireNodeIdx && ps.isTokenNode2 && ps.isTextNode && ps.requireSymbolIdx === -1 && !isKeywordNode(ps.node)
  }

  function nsCanCollectSymbolsInsidePrefixList (ps) {
    return ps.insidePrefixList && ps.isTokenNode2 && ps.isTextNode
  }
  ...
  function parseNs (nodesArr) {
    ...
    while (continueParsingNsForm) {
      ...
      } else if (nsCanCollectRequireSymbolNotInsideListOrVector({ insideRequireForm, insideRequireList, idx, requireNodeIdx, isTokenNode2, isTextNode, requireSymbolIdx, node })) {
        if (!isArray(result.requires)) {
          result.requires = []
        }
      ...
      } else if (nsCanCollectSymbolsInsidePrefixList({ insidePrefixList, isTokenNode2, isTextNode })) {
        if (!isArray(result.requires)) {
          result.requires = []
        }
