...
(function (root, factory) {
  ...
  // isNextLineNodesSlurpedUp
  function yyyyyyyyyyyyyyyyy (nodes, idx) {
  ...
  function findForwardCloserNodes (nodes, idx) {
    const closers = []
    const nodesSize = arraySize(nodes)
    // const firstIdx = idx
    ...
    // console.log(closers.map(n => n.text))

    return closers
  }
  */

  function findForwardCloserNodesIncludeComments (nodes, idx) {
    const closers = []
    const nodesSize = arraySize(nodes)
    // const firstIdx = idx
  ...
  function formatNodes (nodesArr, parsedNs) {
    ...
    while (idx < numNodes) {
      ...
      // Are we on a comment at the end of the line and still inside of a parenStack?
      // Look forward to see if we can close the current parenTrail
      const condition1 = isCommentNode(node) && nextTextNode && isNewlineNode(nextTextNode) && arraySize(parenStack) !== 0
      const condition2 = isNewlineNode(node) && arraySize(parenStack) !== 0
      if (condition1 || condition2) {
        // FIXME: this works to write the closers at the end of the line before a comment
        // but it breaks "downstream" assumptions about things

        // console.log('wrap parens around comment:', node)

        // console.log('nextTextNode:', nextTextNode)
        // console.log('HHHHHH:', nodesArr[inc(idx)])

        // FIXME: make this a throw()?
        // assert(nodesArr[inc(idx)].id === nextTextNode.id, 'comment followed by newline node structure is not what we expect')

        const parenTrailClosers2 = findForwardCloserNodesIncludeComments(nodesArr, inc(idx))

        // console.log('current node:', node)
        // console.log('paren trail closers with comment nodes:')
        // console.log(parenTrailClosers2.map(n => n.text))

        let idx8 = 0
        const numParenTrailClosers = arraySize(parenTrailClosers2)
        ...
        while (idx8 < numParenTrailClosers) {
          const parenTrailCloserNode = parenTrailClosers2[idx8]
        ...
        // add back the space we rtrimmed
        // FIXME: I think we need to look at the whitespace node for this instead of hard-coding a single space here
      ...
      if (currentNodeIsNewline) {
        ...
        const parenTrailClosers = findForwardCloserNodes(nodesArr, inc(idx))

        // console.log(lineIdx)
        // console.log(parenTrailClosers)
        ...
        while (closersIdx < numClosers) {
          ...
          if (isParenCloser(closerNode)) {
            ...
            // zero out this node text so that it will be ignored moving forward
            // closerNode.text = ''
            // closerNode._skipPrintingThisNode = true
            closerNode.text = ''
            closerNode._wasSlurpedUp = true
        ...
        */
        ...
        // FIXME: if all of the nodes on the upcoming line have been slurped up (ie: they were paren closers)
        ...
        // console.log('numSpacesOnNextLine:', numSpacesOnNextLine)

        const allNextLineNodesWereSlurpedUp = yyyyyyyyyyyyyyyyy(nodesArr, inc(idx))
        // console.log('all slurped up?', lineIdx, allNextLineNodesWereSlurpedUp)
