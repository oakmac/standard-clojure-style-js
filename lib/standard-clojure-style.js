...
(function (root, factory) {
  ...
  // searches forward in the nodes array to find the next node that can be the starting ignore node
  // note that this node must be past the ignoreNodeId argument
  // returns the idx of the node if found, null otherwise
  function findIgnoreNodeIdx (allNodes, startIdx, ignoreNodeId) {
    const maxIdx = arraySize(allNodes)
    let idx = startIdx
    let afterIgnoreKeyword = false
  ...
  // // given an opener node, returns the node id of its corresponding closing node
  // function findIdOfClosingNode (openerNode) {
  ...
  function formatNodes (nodesArr, parsedNs) {
    ...
    while (idx < numNodes) {
      ...
      // record original column indexes for the first line
        ...
        // FIXME: this could probably be extracted out into a function
        if (isDiscardNode(node) && nextTextNode) {
          let ignoreStartingIdx = -1
          const nextNextIdx = inc(inc(idx))
          if (isWhitespaceNode(nextTextNode) && nextNextIdx < numNodes) {
            const nextNextTextNode = findNextNodeWithText(nodesArr, nextNextIdx)
            if (isStandardCljIgnoreKeyword(nextNextTextNode)) {
              ignoreStartingIdx = findIgnoreNodeIdx(nodesArr, inc(idx), nextNextTextNode.id)
            }
          } else if (isStandardCljIgnoreKeyword(nextTextNode)) {
            ignoreStartingIdx = findIgnoreNodeIdx(nodesArr, inc(idx), nextTextNode.id)
          }
          // FIXME: we need to handle the #_ {:standard-clj/ignore true} map scenario here

          if (ignoreStartingIdx > 0) {
            const ignoreStartingNode = nodesArr[ignoreStartingIdx]
            // console.log('node to start ignoring on:', nodesArr[ignoreStartingIdx])
            ignoreNodesStartId = ignoreStartingNode.id

            if (ignoreStartingNode.name === 'parens') {
              const closingNode = arrayLast(ignoreStartingNode.children)
              ignoreNodesEndId = closingNode.id
            } else {
              console.log('FIXME: ignore node is not parens, need to handle this')
            }
          }
        }
