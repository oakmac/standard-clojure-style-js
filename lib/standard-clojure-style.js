...
(function (root, factory) {
  ...
  function isDiscardNode (n) {
    return n && n.name === 'marker' && n.text === '#_'
  }

  function isStandardCljIgnoreKeyword (n) {
    return n && n.name === 'token' && n.text === ':standard-clj/ignore'
  }
  ...
  function findNextNodeWithPredicate (allNodes, idx, predFn) {
    const maxIdx = arraySize(allNodes)
  ...
  function findNextNodeWithText2 (allNodes, startIdx, afterNodeId) {
    const maxIdx = arraySize(allNodes)
    let keepSearching = true
    let idx = startIdx
    let beforeAfterNode = false
    ...
    while (continueParsingNsForm) {
      ...
      // collect ns metadata before we hit the nsSymbol
        ...
        if (nextTextNodeIsMetadataKey && node.name === '.close' && node.text === '}') {
          ...
          metadataValueNodeIdx = inc(inc(idx))
        } else if (metadataValueNodeIdx === idx) {
          const metadataObj = {}
      ...
      // collect metadata after the ns symbol
      } else if (nsSymbolIdx > 0 && idx === inc(nsSymbolIdx) && isWhitespaceNode(node)) {
        const nextNode = findNextNodeWithText(nodesArr, inc(idx))
        if (nextNode && nextNode.name === '.open' && nextNode.text === '{') {
          insideNsMetadataHashMap = true
        }
      ...
      if (insideTheIgnoreZone) {
        ...
        if (isStandardCljIgnoreKeyword(node) && idx > 1) {
          ...
          if (isDiscardNode(prevNode1) || (isWhitespaceNode(prevNode1) && isDiscardNode(prevNode2))) {
            ...
            // console.log('next ignore node:', nextIgnoreNode)

            // if parens or brackets or something with children, then find the closing node id
            if (nextIgnoreNode.children && arraySize(nextIgnoreNode.children) > 0) {
            ...
            // console.log('1:', nodesArr[idx - 1])
            // console.log('2:', nodesArr[idx - 2])
            // console.log('3:', nodesArr[idx - 3])
            // console.log('4:', nodesArr[idx - 4])
            // console.log('5:', nodesArr[idx - 5])

            // FIXME: extract this function
            const openingBraceNode = findPrevNodeWithPredicate(nodesArr, idx, (n) => {
              return n && n.name === 'braces' && isArray(n.children) && arraySize(n.children) === 3 &&
              n.children[2].name === '.close' && n.children[2].text === '}'
            })
            ...
            const startIgnoreNode = findNextNodeWithPredicateAfterSpecificNode(nodesArr, idx, hhh, closingBraceNodeId)
            // console.log('start ignore node:', startIgnoreNode)

            const yyy = (n) => {
              // return nodeContainsText(n) && !isWhitespaceNode(n)
              return true
            }
            const firstNodeInsideIgnoreZone = findNextNodeWithPredicateAfterSpecificNode(nodesArr, idx, yyy, startIgnoreNode.id)
            // console.log('first node with text inside ignore zone:', firstNodeInsideIgnoreZone)

            // FIXME: we need to go back to the parent "{" and find it's closing child
            // then we need to find the nextIgnoreNode

            // FIXME: this must be refactored
            // const closingBraceNode = findNextNodeWithPredicate(nodesArr, idx, (n) => {
            //   return n && n.name === '.close' && n.text === '}'
            // })
            // const firstNodeAfterClosingBrace = findNextNodeWithText2(nodesArr, idx, closingBraceNode.id)
            // const nextIgnoreNode = findNextNodeWithText2(nodesArr, idx, firstNodeAfterClosingBrace.id)
