...
(function (root, factory) {
  ...
  function format (inputTxt) {
    ...
    while (idx < numNodes) {
      ...
      // FIXME: do this on another commit
      ...
      if (currentNodeIsNewline) {
        ...
        const nextLineIsOnlyCommentLine = isNextLineACommentLine(nodesArr, inc(idx))
        let nextLineCommentColIdx = -1
        if (nextLineIsOnlyCommentLine) {
          ...
          // console.log('next line is only comment line:', nextLineCommentNode)
          // console.log('current node:', node)
          // console.log('nextLineCommentColIdx:', nextLineCommentColIdx)
        }

        // console.log('idx:', idx, 'zzz' + node.text + 'zzz')
        // console.log(nextLineIsOnlyCommentLine)
        ...
        if (outputTxtContainsChars && !isLastNode) {
          ...
          if (nextLineIsOnlyCommentLine) {
            let idx4 = 0
            const numPrevLineNodeIndexes = arraySize(prevLineNodeIndexes)
            while (idx4 < numPrevLineNodeIndexes) {
              if (nextLineCommentColIdx === prevLineNodeIndexes[idx4]) {
          ...
          } else if (nextLineIsOnlyCommentLine && commentLooksAlignedWithPreviousForm) {
          // } else if (nextLineIsOnlyCommentLine) {
            // numSpaces = 37
          ...
          // console.log('resetting previous line node indexes')
          prevLineNodeIndexes = []
        ...
        // add this colIdx to the previous line node list
        if (!currentNodeIsWhitespace) {
          stackPush(prevLineNodeIndexes, colIdx)
          // console.log('prevLineNodeIndexes:', prevLineNodeIndexes)
        }
