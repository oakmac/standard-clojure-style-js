...
(function (root, factory) {
  ...
  // function getTextFromMetadata (metadataNode) {
  //   let s = ''
  //   recurseAllChildren(rootNode, n => {
  //     if (isNonBlankString(n.text)) {
  //       s = strConcat(s, n.text)
  ...
  // function getNodeIdOfLastCloser (node) {
  //   let lastId = null

  //   recurseAllChildren(node, (n) => {
  //     if (isParenCloser(n)) {
  //       lastId = n.id
  //     }
  //   })
  ...
  function parseNs (nodesArr) {
    ...
    while (continueParsingNsForm) {
      ...
      if (node.name === 'meta') {
        ...
        // console.log('aaaa:', node.children[1])
        // console.log('bbbb:', node.children[2])

        // I think this works, but we need to test more
        console.log('node id of last closer:', getNodeIdOfLastCloser(node.children[1]))

        const metadataCloserNodeId = getNodeIdOfLastCloser(node.children[1])

        // TODO: put this into a separate function
        const metadataMarker = getTextFromRootNode(node.children[0])
        const metadataBodyTxt = getTextFromRootNode(node.children[1])

        const metadataTxt = strConcat(metadataMarker, metadataBodyTxt)
        // console.log('zzzz' + metadataTxt + 'zzzz')

        // TODO: find the node id of the body closing node here
        // we can skip all nodes until we hit that node (in the outer loop)
