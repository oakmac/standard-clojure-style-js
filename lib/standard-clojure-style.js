...
(function (root, factory) {
  ...
  // returns the index of the last occurrence of needle inside of s
  // returns -1 if needle is not found
  function strLastIndexOf (s, needle) {
    return s.lastIndexOf(needle)
  }

  // returns the index of the first occurrence of needle inside of s,
  // starting the search at fromIdx
  // returns -1 if needle is not found
  function strIndexOfFrom (s, needle, fromIdx) {
    return s.indexOf(needle, fromIdx)
  }
  ...
  // does String s contain any non-whitespace characters?
  // equivalent to strTrim(s) !== '', but does not allocate a new String
  function strHasNonWhitespaceChars (s) {
    return /\S/.test(s)
  }
  ...
  function Node (opts) {
    return {
      ...
      text: opts.text,
      // these fields are used by the formatNodes function
      // NOTE: predeclaring them here keeps all Node objects the same "shape",
      // which allows JS engines to optimize property access
      _origColIdx: -1,
      _printedColIdx: -1,
      _printedLineIdx: -1,
      _wasSlurpedUp: false
  ...
  function Named (opts) {
    return {
      parse: (txt, pos) => {
        // lazily resolve a string parser reference to the parser object once,
        // then cache it for future calls
        if (isString(opts.parser)) {
          opts.parser = getParser(opts.parser)
        }
        const node = opts.parser.parse(txt, pos)
  ...
  function Regex (opts) {
    // Convert the pattern to a "sticky" regex (y flag) so we can match at any
    // position inside of txt directly. This avoids allocating a copy of the
    // entire remaining input (substr) on every parse attempt, which is a
    // significant performance + garbage collection win on large files.
    // NOTE: the y flag anchors the match to .lastIndex, so we must remove the
    // '^' input boundary assertion from the pattern (with y, '^' would assert
    // the start of the entire input and always fail for pos > 0).
    let regexSrc = opts.regex.source
    if (strStartsWith(regexSrc, '^')) {
      regexSrc = substr(regexSrc, 1, -1)
    }
    const stickyRegex = new RegExp(regexSrc, 'y')

    return {
      name: opts.name,
      parse: (txt, pos) => {
        stickyRegex.lastIndex = pos
        const result = stickyRegex.exec(txt)
  ...
  function Choice (opts) {
    return {
      parse: (txt, pos) => {
        ...
        const numParsers = arraySize(opts.parsers)
        while (i < numParsers) {
          // lazily resolve string parser references to their parser objects
          // once, then cache them for future calls
          if (isString(opts.parsers[i])) {
            opts.parsers[i] = getParser(opts.parsers[i])
          }
          const possibleNode = opts.parsers[i].parse(txt, pos)
  ...
  // lookup table of literal whitespace characters
  // NOTE: keep this in sync with whitespaceCommons + whitespaceUnicodes above
  const whitespaceCharTbl = {
    ' ': true,
    ',': true,
    '\n': true,
    '\r': true,
  ...
  // _gap is equivalent to Choice({ parsers: ['_ws', 'comment', 'discard'] }),
  // but instead of attempting each parser in order we dispatch on the first
  // character. The three parsers cannot match the same first character
  // (whitespace / ';' / '#'), so this is purely a performance optimization.
  parsers._gap = {
    parse: (txt, pos) => {
      const ch = charAt(txt, pos)
      if (whitespaceCharTbl[ch]) {
        return parsers._ws.parse(txt, pos)
      } else if (ch === ';') {
        return parsers.comment.parse(txt, pos)
      } else if (ch === '#') {
        return parsers.discard.parse(txt, pos)
      } else {
        return null
      }
    }
  ...
  // _form is equivalent to
  // Choice({ parsers: ['token', 'string', 'parens', 'brackets', 'braces', 'wrap', 'meta', 'tagged'] }),
  // but instead of attempting each parser in order we dispatch on the first
  // character, which decides the correct parser in every case except '#'.
  // This is purely a performance optimization: the token parser cannot match
  // any of the dispatched characters (they are all in tokenHeadChars), so
  // routing them directly to their parser preserves the original semantics.
  parsers._hashForm = Choice({ parsers: ['token', 'string', 'parens', 'braces', 'wrap', 'meta', 'tagged'] })

  parsers._form = {
    parse: (txt, pos) => {
      const ch = charAt(txt, pos)
      if (ch === '(') {
        return parsers.parens.parse(txt, pos)
      } else if (ch === '[') {
        return parsers.brackets.parse(txt, pos)
      } else if (ch === '{') {
        return parsers.braces.parse(txt, pos)
      } else if (ch === '"') {
        return parsers.string.parse(txt, pos)
      } else if (ch === '@' || ch === "'" || ch === '`' || ch === '~') {
        return parsers.wrap.parse(txt, pos)
      } else if (ch === '^') {
        return parsers.meta.parse(txt, pos)
      } else if (ch === '#') {
        // '#' is ambiguous (#"str", #{set}, #::{ns-map}, #(anon-fn), #?(cond),
        // #'var, #^meta, ##Inf, #tagged), so fall back to trying the parsers
        // in their original order (minus brackets, which can never match '#')
        return parsers._hashForm.parse(txt, pos)
      } else {
        return parsers.token.parse(txt, pos)
      }
    }
  }
  ...
  function isMapLiteralNode (n) {
    if (!n || n.name !== 'braces' || !isArray(n.children)) {
      return false
    }

    const numChildren = arraySize(n.children)
    return numChildren >= 2 &&
      isMapLiteralOpener(n.children[0]) &&
      n.children[numChildren - 1].name === '.close' &&
      n.children[numChildren - 1].text === '}'
  }

  function isVectorLiteralNode (n) {
    if (!n || n.name !== 'brackets' || !isArray(n.children)) {
      return false
    }

    const numChildren = arraySize(n.children)
    return numChildren >= 2 &&
      isVectorLiteralOpener(n.children[0]) &&
      n.children[numChildren - 1].name === '.close' &&
      n.children[numChildren - 1].text === ']'
  }

  function isMetaMarker (n) {
    return n && n.name === '.marker' && (n.text === '^' || n.text === '#^')
  }

  function isStringOpener (n) {
    return n && n.name === '.open' && (n.text === '"' || n.text === '#?"')
  }

  function isStringCloser (n) {
    return n && n.name === '.close' && n.text === '"'
  }
  ...
  // these are all of the possible :gen-class keywords that can have a symbol or string value
  ...
  function parseGenClassConstructors (mapNode) {
    const constructorErrorMessage = ':gen-class :constructors must be a map of parameter and super-parameter vectors.'
    const pairs = getGenClassMapPairs(
      mapNode,
      ':gen-class :constructors must contain parameter and super-parameter vector pairs.'
    )
    const constructors = []
    const numPairs = arraySize(pairs)
    let pairIdx = 0
    while (pairIdx < numPairs) {
      const parameterTypesForm = splitGenClassMetadataForm(pairs[pairIdx][0])
      const parameterTypesNode = parameterTypesForm.bodyNode
      const superParameterTypesNode = pairs[pairIdx][1]

      if (!isVectorLiteralNode(parameterTypesNode) || !isVectorLiteralNode(superParameterTypesNode)) {
        throw new Error(constructorErrorMessage)
      }

      const constructor = {}
      if (isStringWithChars(parameterTypesForm.metadata)) {
        constructor.metadata = parameterTypesForm.metadata
      }
      constructor.parameterTypes = parseGenClassTypeVector(parameterTypesNode, constructorErrorMessage)
      constructor.superParameterTypes = parseGenClassTypeVector(superParameterTypesNode, constructorErrorMessage)

      stackPush(constructors, constructor)
      pairIdx = inc(pairIdx)
    }
  ...
  function getDirectFormNodesFromCollection (collectionNode) {
    const formNodes = []
    const numCollectionChildren = arraySize(collectionNode.children)

    if (numCollectionChildren === 3 && collectionNode.children[1].name === '.body') {
      const bodyChildren = collectionNode.children[1].children
      const numBodyChildren = arraySize(bodyChildren)
      let idx = 0
      while (idx < numBodyChildren) {
        const childNode = bodyChildren[idx]
        if (!isWhitespaceNode(childNode) && !isCommentNode(childNode)) {
          stackPush(formNodes, childNode)
        }
        idx = inc(idx)
      }
    }
  ...
  function getGenClassSymbolText (node) {
    if (!node) return null

    if (isTokenNode(node)) {
      return node.text
    } else if (isStringNode(node)) {
      return strConcat3('"', getTextFromStringNode(node), '"')
    } else if (node.name === 'meta') {
      return getTextFromRootNode(node)
    }
    return null
  }

  function splitGenClassMetadataForm (node) {
    if (!node || node.name !== 'meta' || !isArray(node.children)) {
      return { bodyNode: node, metadata: null }
    }
    ...
    while (idx < numChildren) {
      ...
      if (childNode.name === '.body') {
        if (isArray(childNode.children) && arraySize(childNode.children) === 1) {
          bodyNode = childNode.children[0]
        }
        idx = numChildren
      } else {
        metadata = strConcat(metadata, getTextFromRootNode(childNode))
        idx = inc(idx)
      }
    }
  ...
  function parseGenClassTypeVector (vectorNode, errorMessage) {
    const typeNodes = getDirectFormNodesFromCollection(vectorNode)
    const types = []
    const numTypeNodes = arraySize(typeNodes)
    let idx = 0
    while (idx < numTypeNodes) {
      const typeName = getGenClassSymbolText(typeNodes[idx])
      if (!typeName) {
        throw new Error(errorMessage)
      }
      stackPush(types, typeName)
      idx = inc(idx)
    }
    return types
  }

  function parseGenClassParameterTypes (vectorNode) {
    return parseGenClassTypeVector(
      vectorNode,
      ':gen-class :methods parameter types must be symbols inside a vector.'
    )
  }

  function parseGenClassMethods (vectorNode) {
    const methodNodes = getDirectFormNodesFromCollection(vectorNode)
    const methods = []
    const numMethodNodes = arraySize(methodNodes)
    let methodIdx = 0
    while (methodIdx < numMethodNodes) {
      const methodForm = splitGenClassMetadataForm(methodNodes[methodIdx])
      const methodNode = methodForm.bodyNode
      if (!isVectorLiteralNode(methodNode)) {
        throw new Error(':gen-class :methods must be a vector of method vectors.')
      }

      const methodFormNodes = getDirectFormNodesFromCollection(methodNode)
      if (arraySize(methodFormNodes) !== 3) {
        throw new Error(':gen-class :methods entries must contain a name, parameter-types vector, and return type.')
      }

      const name = getGenClassSymbolText(methodFormNodes[0])
      const parameterTypesNode = methodFormNodes[1]
      const returnType = getGenClassSymbolText(methodFormNodes[2])
      if (!name) {
        throw new Error(':gen-class :methods method names must be symbols.')
      }
      if (!isVectorLiteralNode(parameterTypesNode)) {
        throw new Error(':gen-class :methods parameter types must be provided in a vector.')
      }
      if (!returnType) {
        throw new Error(':gen-class :methods return types must be symbols.')
      }

      const method = {}
      if (isStringWithChars(methodForm.metadata)) {
        method.metadata = methodForm.metadata
      }
      method.name = name
      method.parameterTypes = parseGenClassParameterTypes(parameterTypesNode)
      method.returnType = returnType

      stackPush(methods, method)
      methodIdx = inc(methodIdx)
    }

    return methods
  }

  function getGenClassMapPairs (mapNode, errorMessage) {
    const formNodes = getDirectFormNodesFromCollection(mapNode)
    if (arraySize(formNodes) % 2 !== 0) {
      throw new Error(errorMessage)
    }

    const pairs = []
    let idx = 0
    while (idx < arraySize(formNodes)) {
      stackPush(pairs, [formNodes[idx], formNodes[inc(idx)]])
      idx = idx + 2
    }
    return pairs
  }

  function parseGenClassExposes (mapNode) {
    const pairs = getGenClassMapPairs(
      mapNode,
      ':gen-class :exposes must contain protected-field and accessor-map pairs.'
    )
    const exposes = []
    const numPairs = arraySize(pairs)
    let pairIdx = 0
    while (pairIdx < numPairs) {
      const fieldName = getGenClassSymbolText(pairs[pairIdx][0])
      const accessorsNode = pairs[pairIdx][1]
      if (!fieldName) {
        throw new Error(':gen-class :exposes protected field names must be symbols.')
      }
      if (!isMapLiteralNode(accessorsNode)) {
        throw new Error(':gen-class :exposes field values must be maps containing :get and/or :set.')
      }

      const accessorPairs = getGenClassMapPairs(
        accessorsNode,
        ':gen-class :exposes accessor maps must contain keyword and method-name pairs.'
      )
      const expose = { fieldName }
      const seenAccessors = {}
      const numAccessorPairs = arraySize(accessorPairs)
      let accessorIdx = 0
      while (accessorIdx < numAccessorPairs) {
        const accessorKeyNode = accessorPairs[accessorIdx][0]
        const accessorNameNode = accessorPairs[accessorIdx][1]
        const accessorKey = isTokenNode(accessorKeyNode) ? accessorKeyNode.text : null
        const accessorName = getGenClassSymbolText(accessorNameNode)

        if (accessorKey !== ':get' && accessorKey !== ':set') {
          throw new Error(':gen-class :exposes accessor keys must be :get or :set.')
        }
        if (seenAccessors[accessorKey]) {
          throw new Error(':gen-class :exposes accessor maps cannot repeat :get or :set.')
        }
        if (!accessorName) {
          throw new Error(':gen-class :exposes accessor method names must be symbols.')
        }

        if (accessorKey === ':get') {
          expose.getter = accessorName
        } else {
          expose.setter = accessorName
        }
        seenAccessors[accessorKey] = true
        accessorIdx = inc(accessorIdx)
      }

      stackPush(exposes, expose)
      pairIdx = inc(pairIdx)
    }
    return exposes
  }

  function parseGenClassExposesMethods (mapNode) {
    const pairs = getGenClassMapPairs(
      mapNode,
      ':gen-class :exposes-methods must contain superclass-method and exposed-name pairs.'
    )
    const exposesMethods = []
    const numPairs = arraySize(pairs)
    let pairIdx = 0
    while (pairIdx < numPairs) {
      const superMethodName = getGenClassSymbolText(pairs[pairIdx][0])
      const exposedName = getGenClassSymbolText(pairs[pairIdx][1])
      if (!superMethodName || !exposedName) {
        throw new Error(':gen-class :exposes-methods names must be symbols.')
      }

      stackPush(exposesMethods, { superMethodName, exposedName })
      pairIdx = inc(pairIdx)
    }
    return exposesMethods
  }
  ...
  // returns the metadata forms attached by a meta node, including their
  // markers, in source order
  function getMetadataStringsFromMetaNode (metaNode) {
    const metadata = []
    let marker = null
    const numChildren = arraySize(metaNode.children)
    let idx = 0

    while (idx < numChildren) {
      const childNode = metaNode.children[idx]

      if (isMetaMarker(childNode)) {
        marker = childNode.text
      } else if (marker && childNode.name === '.meta') {
        stackPush(metadata, strConcat(marker, getTextFromRootNode(childNode)))
        marker = null
      }
  ...
  // returns the wrapped body node from a meta node
  function getBodyNodeFromMetaNode (metaNode) {
    const numChildren = arraySize(metaNode.children)
    let idx = 0
  ...
  // Searches forward in the nodes array to find the next meaningful text node,
  // explicitly skipping over metadata markers (e.g., ^, ^{...}) and reader
  // discards (#_).
  //
  // Returns the Node if found, null otherwise.
  function findNextTextNodeSkippingMeta (nodesArr, startIdx) {
    const maxIdx = arraySize(nodesArr)
    let idx = startIdx
    ...
    while (keepSearching) {
      if (idx >= maxIdx) {
        ...
        // We only evaluate text-bearing non-whitespace, non-comment nodes
        if (nodeContainsText(node) && !isWhitespaceNode(node) && !isCommentNode(node)) {
          if (isParenOpener(node)) {
            if (stringDepth === 0) {
              if (formsToSkip > 0 && parenDepth === 0) {
                parenDepth = 1
              } else if (parenDepth > 0) {
                parenDepth = inc(parenDepth)
              } else {
                resultNode = node
                keepSearching = false
              }
            ...
            if (stringDepth === 0) {
              if (parenDepth > 0) {
                parenDepth = dec(parenDepth)
                if (parenDepth === 0) {
                  formsToSkip = dec(formsToSkip)
                }
              } else {
                resultNode = node
                keepSearching = false
          ...
          } else if (isStringOpener(node)) {
            if (formsToSkip > 0 && parenDepth === 0 && stringDepth === 0) {
              stringDepth = 1
            } else if (parenDepth > 0 || stringDepth > 0) {
              stringDepth = inc(stringDepth)
            } else {
              resultNode = node
              keepSearching = false
            }
          } else if (isStringCloser(node)) {
            if (stringDepth > 0) {
              stringDepth = dec(stringDepth)
              if (stringDepth === 0 && parenDepth === 0) {
                formsToSkip = dec(formsToSkip)
              }
            } else {
              resultNode = node
          ...
          // It's a token, marker, or string body
            if (parenDepth > 0 || stringDepth > 0) {
            // inside a form we are currently skipping
            } else if (isMetaMarker(node) || isDiscardNode(node)) {
            // Found a marker: we must skip the next complete form
              formsToSkip = inc(formsToSkip)
            } else if (formsToSkip > 0) {
            // We just skipped a single-token form (e.g. `long`, `:private`)
              formsToSkip = dec(formsToSkip)
            } else {
              resultNode = node
              keepSearching = false
  ...
  function isNewlineNodeWithCommaOnNextLine (n) {
    return !!(n && isNewlineNode(n) && txtHasCommasAfterNewline(n.text))
  }
  ...
  function numSpacesAfterNewline (newlineNode) {
    return dec(strLen(newlineNode.text) - strLastIndexOf(newlineNode.text, '\n'))
  }
  ...
  // is ch a space or comma character?
  ...
  function removeTrailingWhitespace (txt) {
    let endIdx = strLen(txt)
    while (endIdx > 0 && isSpaceOrComma(charAt(txt, dec(endIdx)))) {
      endIdx = dec(endIdx)
    }
    return substr(txt, 0, endIdx)
  }

  function removeCharsUpToNewline (txt) {
    return substr(txt, inc(strLastIndexOf(txt, '\n')), -1)
  }

  function txtHasCommasAfterNewline (s) {
    const lastNewlineIdx = strLastIndexOf(s, '\n')
    if (lastNewlineIdx === -1) {
      return false
    }
    return strIndexOfFrom(s, ',', lastNewlineIdx) !== -1
  }
  ...
  function numSpacesForIndentation (wrappingOpener) {
    if (!wrappingOpener) {
      ...
      const nextNodeAfterOpener = wrappingOpener._nextWithTextSkippingMeta
  ...
  function parseNs (nodesArr) {
    ...
    let genClassValueLastNodeId = -1
    let pendingRequireMetadata = []
    let pendingReaderCommentLineNo = -1
    ...
    while (continueParsingNsForm) {
      ...
      // an end-of-line comment may merge onto a reader comment collected earlier on this
      // same line, but only if no libspec has been parsed in between: parsing a libspec
      // flushes singleLineComments onto that require, which closes the merge window
      // (a comment after the libspec belongs to the libspec as commentAfter, not to the
      // reader comment as commentsAbove)
      const readerCommentMergeWindowIsOpen = insideRequireForm && pendingReaderCommentLineNo === lineNo && arraySize(singleLineComments) > 0
      ...
      if (skipNodesUntilWeReachThisId > 0) {
        if (node.id === skipNodesUntilWeReachThisId) {
          ...
          if (node.id === genClassValueLastNodeId) {
            genClassValueLineNo = lineNo
            genClassValueLastNodeId = -1
          }
        }

        // collect metadata attached to the next :require libspec. The meta node
        // already groups stacked metadata and its wrapped body, so skip only the
        // metadata children and let the normal require parser consume the body.
      } else if (insideRequireForm && requireSymbolIdx === -1 && node.name === 'meta') {
        const metadata = getMetadataStringsFromMetaNode(node)
        const numMetadata = arraySize(metadata)
        let metadataIdx = 0
        while (metadataIdx < numMetadata) {
          stackPush(pendingRequireMetadata, metadata[metadataIdx])
          metadataIdx = inc(metadataIdx)
        }

        const bodyNode = getBodyNodeFromMetaNode(node)
        if (bodyNode) {
          skipNodesUntilWeReachThisId = bodyNode.id
        }
      ...
      // collect a reader comment that appears inline after the (:require keyword,
      // before any libspec (ie: on the same line as the start of the :require form)
      } else if (insideRequireForm && activeRequireIdx < 0 && requireFormLineNo === lineNo && requireSymbolIdx === -1 && referIdx === -1 && renameIdx === -1 && isReaderCommentNode2) {
        stackPush(singleLineComments, getTextFromRootNode(node))
        pendingReaderCommentLineNo = lineNo

      // merge this comment onto the pending reader comment so they travel together during sorting
      } else if (readerCommentMergeWindowIsOpen && isCommentNode2) {
        const lastCommentIdx = dec(arraySize(singleLineComments))
        singleLineComments[lastCommentIdx] = strConcat3(singleLineComments[lastCommentIdx], ' ', node.text)
        // reset so a comment on a later line cannot merge onto this reader comment
        pendingReaderCommentLineNo = -1

      // collect a reader comment that starts its own line
      } else if (insideNsForm && idx > nsNodeIdx && prevNodeIsNewline && isReaderCommentNode2) {
        stackPush(singleLineComments, getTextFromRootNode(node))
        // NOTE: record the line number so that an end-of-line comment following this
        // reader comment can be merged onto it by the branch above
        pendingReaderCommentLineNo = lineNo
      ...
      // collect :require symbol not inside of a list / vector
        ...
        const requireObj = {}
        if (arraySize(pendingRequireMetadata) > 0) {
          requireObj.metadata = pendingRequireMetadata
          pendingRequireMetadata = []
        }
        requireObj.symbol = node.text
      ...
      // collect symbols inside of a prefix list
        ...
        requireObj.prefixListId = currentPrefixListId
        requireObj.symbol = namespace
        stackPush(result.requires, requireObj)
        activeRequireIdx = inc(activeRequireIdx)
      ...
      // collect :import classes inside of a list or vector
      ...
      } else if (insideGenClass && insideGenClassImplements && idx > genClassNodeIdx && genClassToggle === 1 && genClassKeyStr === 'implements' && (isTokenNode2 || isStringNode(node) || node.name === 'meta')) {
        const symbol = getGenClassSymbolText(node)
        if (!symbol) {
          throw new Error(':gen-class :implements values must be symbols or strings.')
        }
        ...
        if (!isTokenNode2) {
          const lastNode = getLastChildNodeWithText(node)
          skipNodesUntilWeReachThisId = lastNode.id
        }
      ...
      // :gen-class :prefix value
        ...
        result.genClass.prefix.value = getGenClassSymbolText(node)
        genClassToggle = 0

        const lastNode = getLastChildNodeWithText(node)
        skipNodesUntilWeReachThisId = lastNode.id
        genClassValueLastNodeId = lastNode.id

      // :gen-class name-like value with metadata
      // NOTE: :name is the only key where metadata is meaningful to gen-class (class
      // annotations), but metadata is legal reader syntax on any of these values and
      // must be preserved, not dropped
      } else if (insideGenClass && idx > genClassNodeIdx && genClassToggle === 1 && isGenClassNameKey(genClassKeyStr) && node.name === 'meta') {
        const valueForm = splitGenClassMetadataForm(node)
        const value = getGenClassSymbolText(valueForm.bodyNode)
        if (!value) {
          throw new Error(strConcat3(':gen-class :', genClassKeyStr, ' must be a symbol or string.'))
        }

        if (isStringWithChars(valueForm.metadata)) {
          result.genClass[genClassKeyStr].metadata = valueForm.metadata
        }
        result.genClass[genClassKeyStr].value = value
        genClassToggle = 0
      ...
      // other :gen-class name-like values
      } else if (insideGenClass && idx > genClassNodeIdx && genClassToggle === 1 && isGenClassNameKey(genClassKeyStr) && (isTokenNode2 || isStringNode(node))) {
        result.genClass[genClassKeyStr].value = getGenClassSymbolText(node)
        genClassToggle = 0
      ...
      // :gen-class :constructors value
      } else if (insideGenClass && idx > genClassNodeIdx && genClassToggle === 1 && genClassKeyStr === 'constructors' && isMapLiteralNode(node)) {
        result.genClass.constructors.value = parseGenClassConstructors(node)
        genClassToggle = 0
      ...
      // :gen-class :methods value
      } else if (insideGenClass && idx > genClassNodeIdx && genClassToggle === 1 && genClassKeyStr === 'methods' && isVectorLiteralNode(node)) {
        result.genClass.methods.value = parseGenClassMethods(node)
        genClassToggle = 0
      ...
      // invalid :gen-class :methods value
      } else if (insideGenClass && idx > genClassNodeIdx && genClassToggle === 1 && genClassKeyStr === 'methods' && nodeContainsTextAndNotWhitespace(node)) {
        throw new Error(':gen-class :methods must be a vector of method vectors.')

      // :gen-class :exposes value
      } else if (insideGenClass && idx > genClassNodeIdx && genClassToggle === 1 && genClassKeyStr === 'exposes' && isMapLiteralNode(node)) {
        result.genClass.exposes.value = parseGenClassExposes(node)
        genClassToggle = 0
      ...
      // invalid :gen-class :exposes value
      } else if (insideGenClass && idx > genClassNodeIdx && genClassToggle === 1 && genClassKeyStr === 'exposes' && nodeContainsTextAndNotWhitespace(node)) {
        throw new Error(':gen-class :exposes must be a map of protected-field names to accessor maps.')

      // :gen-class :exposes-methods value
      } else if (insideGenClass && idx > genClassNodeIdx && genClassToggle === 1 && genClassKeyStr === 'exposes-methods' && isMapLiteralNode(node)) {
        result.genClass['exposes-methods'].value = parseGenClassExposesMethods(node)
        genClassToggle = 0
      ...
      // invalid :gen-class :exposes-methods value
      } else if (insideGenClass && idx > genClassNodeIdx && genClassToggle === 1 && genClassKeyStr === 'exposes-methods' && nodeContainsTextAndNotWhitespace(node)) {
        throw new Error(':gen-class :exposes-methods must be a map of superclass-method names to exposed names.')

      // :main, :load-impl-ns
      } else if (insideGenClass && idx > genClassNodeIdx && isTextNode && isTokenNode2 && genClassToggle === 1 && isGenClassBooleanKey(genClassKeyStr)) {
        if (node.text === 'true') {
  ...
  function formatRequireLine (req, initialIndentation, metadataInline) {
    let outTxt = ''

    let requireIndentation = initialIndentation

    outTxt = printCommentsAbove(outTxt, req.commentsAbove, initialIndentation)

    if (isArray(req.metadata) && arraySize(req.metadata) > 0) {
      outTxt = strConcat(outTxt, initialIndentation)
      outTxt = strConcat(outTxt, strJoin(req.metadata, ' '))
      if (metadataInline) {
    ...
    outTxt = strConcat(outTxt, requireIndentation)
    ...
    if (isString(req.default) && req.default !== '') {
  ...
  function formatNs (ns) {
    ...
    if (numRequireMacros > 0) {
      ...
      while (rmIdx < numRequireMacros) {
        ...
        outTxt = strConcat(outTxt, formatRequireLine(rm, rmIndentation, false))
    ...
    if (numRequires > 0) {
      ...
      while (requiresIdx < numRequires) {
        ...
        if (!req.platform || allRequiresUnderOnePlatform) {
          outTxt = strConcat(outTxt, formatRequireLine(req, requireLineIndentation, false))
      ...
      if (!allRequiresUnderOnePlatform) {
        ...
        if (useStandardReaderConditional) {
          while (platformIdx < numPlatforms) {
            ...
            outTxt = strConcat(outTxt, formatRequireLine(req, '', true))
          ...
          while (platformIdx < numPlatforms) {
            ...
            while (reqIdx2 < numFilteredReqs) {
              ...
              if (printedFirstReqLine) {
                outTxt = strConcat(outTxt, formatRequireLine(req, '        ', false))
              } else {
      ...
      if (ns.genClass.isEmpty) {
        ...
        while (idx3 < numGenClassKeys) {
          ...
          if (genClassValue) {
            ...
            } else if (genClassKey === 'constructors') {
              const constructors = genClassValue.value
              const numConstructors = arraySize(constructors)

              if (numConstructors === 0) {
                outTxt = strConcat(outTxt, ' {}')
              } else {
                const constructorIndentationLevel = genClassValueIndentationLevel + strLen(genClassKey) + 3
                const constructorIndentationStr = repeatString(' ', constructorIndentationLevel)
                let constructorIdx = 0
                while (constructorIdx < numConstructors) {
                  const constructor = constructors[constructorIdx]
                  if (constructorIdx === 0) {
                    outTxt = strConcat(outTxt, ' {')
                  } else {
                    outTxt = strConcat3(outTxt, '\n', constructorIndentationStr)
                  }

                  if (constructor.metadata) {
                    outTxt = strConcat3(outTxt, constructor.metadata, ' ')
                  }

                  outTxt = strConcat3(outTxt, '[', strJoin(constructor.parameterTypes, ' '))
                  outTxt = strConcat3(outTxt, '] [', strJoin(constructor.superParameterTypes, ' '))
                  outTxt = strConcat(outTxt, ']')

                  constructorIdx = inc(constructorIdx)
                }
                outTxt = strConcat(outTxt, '}')
              }
            } else if (genClassKey === 'methods') {
              const methods = genClassValue.value
              const numMethods = arraySize(methods)

              if (numMethods === 0) {
                outTxt = strConcat(outTxt, ' []')
              } else {
                const methodIndentationLevel = genClassValueIndentationLevel + strLen(genClassKey) + 3
                const methodIndentationStr = repeatString(' ', methodIndentationLevel)
                let methodIdx = 0
                while (methodIdx < numMethods) {
                  ...
                  if (methodIdx === 0) {
                    ...
                    outTxt = strConcat3(outTxt, '\n', methodIndentationStr)
                  }

                  if (method.metadata) {
                    outTxt = strConcat3(outTxt, method.metadata, ' ')
                  }

                  outTxt = strConcat3(outTxt, '[', method.name)
                  outTxt = strConcat3(outTxt, ' [', strJoin(method.parameterTypes, ' '))
                  outTxt = strConcat3(outTxt, '] ', method.returnType)
                  outTxt = strConcat(outTxt, ']')
            ...
            } else if (genClassKey === 'exposes') {
              const exposes = genClassValue.value
              const numExposes = arraySize(exposes)

              if (numExposes === 0) {
                outTxt = strConcat(outTxt, ' {}')
              } else {
                const exposesIndentationLevel = genClassValueIndentationLevel + strLen(genClassKey) + 3
                const exposesIndentationStr = repeatString(' ', exposesIndentationLevel)
                let exposeIdx = 0
                while (exposeIdx < numExposes) {
                  const expose = exposes[exposeIdx]
                  if (exposeIdx === 0) {
                    outTxt = strConcat(outTxt, ' {')
                  } else {
                    outTxt = strConcat3(outTxt, '\n', exposesIndentationStr)
                  }

                  outTxt = strConcat3(outTxt, expose.fieldName, ' {')
                  if (expose.getter) {
                    outTxt = strConcat3(outTxt, ':get ', expose.getter)
                  }
                  if (expose.setter) {
                    if (expose.getter) {
                      outTxt = strConcat(outTxt, ' ')
                    }
                    outTxt = strConcat3(outTxt, ':set ', expose.setter)
                  }
                  outTxt = strConcat(outTxt, '}')

                  exposeIdx = inc(exposeIdx)
                }
                outTxt = strConcat(outTxt, '}')
              }
            } else if (genClassKey === 'exposes-methods') {
              const exposesMethods = genClassValue.value
              const numExposesMethods = arraySize(exposesMethods)

              if (numExposesMethods === 0) {
                outTxt = strConcat(outTxt, ' {}')
              } else {
                const exposesMethodsIndentationLevel = genClassValueIndentationLevel + strLen(genClassKey) + 3
                const exposesMethodsIndentationStr = repeatString(' ', exposesMethodsIndentationLevel)
                let exposesMethodIdx = 0
                while (exposesMethodIdx < numExposesMethods) {
                  const exposesMethod = exposesMethods[exposesMethodIdx]
                  if (exposesMethodIdx === 0) {
                    outTxt = strConcat(outTxt, ' {')
                  } else {
                    outTxt = strConcat3(outTxt, '\n', exposesMethodsIndentationStr)
                  }

                  outTxt = strConcat3(outTxt, exposesMethod.superMethodName, ' ')
                  outTxt = strConcat(outTxt, exposesMethod.exposedName)
                  exposesMethodIdx = inc(exposesMethodIdx)
                }
                outTxt = strConcat(outTxt, '}')
              }
            } else if (genClassValue.metadata) {
              outTxt = strConcat3(outTxt, ' ', genClassValue.metadata)
    ...
    while (idx < numNodes) {
      ...
      if (insideTheIgnoreZone) {
        ...
        if (isParenOpener(node)) {
          ...
          parenStackNode._nextWithTextSkippingMeta = findNextTextNodeSkippingMeta(nodesArr, inc(idx))
        ...
        if (currentNodeIsNewline) {
          ...
          if (outputTxtContainsChars) {
            ...
            if (nextLineContainsOnlyOneComment && !isDoubleNewline) {
              let idx2 = 0
            ...
            if (strHasNonWhitespaceChars(lineTxt)) {
              outTxt = strConcat(outTxt, lineTxt)
            }
  ...
  if (exportInternalFnsForTesting) {
    ...
    API._strLastIndexOf = strLastIndexOf
    API._strIndexOfFrom = strIndexOfFrom
    API._strHasNonWhitespaceChars = strHasNonWhitespaceChars
    ...
    API._numSpacesAfterNewline = numSpacesAfterNewline
    API._isNewlineNodeWithCommaOnNextLine = isNewlineNodeWithCommaOnNextLine
    API._isSpaceOrComma = isSpaceOrComma
    ...
    API._findNextNodeWithText = findNextNodeWithText
    API._findNextTextNodeSkippingMeta = findNextTextNodeSkippingMeta
