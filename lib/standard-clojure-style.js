...
(function (root, factory) {
  ...
  // FIXME: add more :gen-class keywords here
  function isGenClassKeyword (node) {
    return node && isString(node.text) && (node.text === ':name' || node.text === ':main' || node.text === ':state' || node.text === ':init')
  }
  ...
  function parseNs (nodesArr) {
    ...
    while (idx < numNodes) {
      ...
      // :gen-class value
        ...
        if (genClassKeyStr === 'name' || genClassKeyStr === 'state' || genClassKeyStr === 'init') {
          result.genClass[genClassKeyStr].value = node.text
          genClassToggle = 0
        } else if (genClassKeyStr === 'main' && node.text === 'true') {
          result.genClass[genClassKeyStr].value = true
          genClassToggle = 0
        } else if (genClassKeyStr === 'main' && node.text === 'false') {
