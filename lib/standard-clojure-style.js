...
(function (root, factory) {
  ...
  function getTokenTextsFromNode (rootNode) {
    const tokens = []
    recurseAllChildren(rootNode, n => {
      if (isTokenNode(n) && isStringWithChars(n.text)) {
        stackPush(tokens, n.text)
      }
    })
    return tokens
  }
  ...
  function parseGenClassConstructors (mapNode) {
    const vectorNodes = []
    const numMapChildren = arraySize(mapNode.children)

    // A non-empty map has a .body node between its opening and closing braces.
    if (numMapChildren === 3 && mapNode.children[1].name === '.body') {
      const mapBodyChildren = mapNode.children[1].children
      const numMapBodyChildren = arraySize(mapBodyChildren)
      let idx = 0
      while (idx < numMapBodyChildren) {
        const childNode = mapBodyChildren[idx]
        if (childNode.name === 'brackets') {
          stackPush(vectorNodes, childNode)
        } else if (!isWhitespaceNode(childNode) && !isCommentNode(childNode)) {
          throw new Error(':gen-class :constructors must be a map of parameter and super-parameter vectors.')
        }
        idx = inc(idx)
      }
    }

    if (arraySize(vectorNodes) % 2 !== 0) {
      throw new Error(':gen-class :constructors must contain parameter and super-parameter vector pairs.')
    }

    const constructors = []
    let vectorIdx = 0
    while (vectorIdx < arraySize(vectorNodes)) {
      stackPush(constructors, {
        parameterTypes: getTokenTextsFromNode(vectorNodes[vectorIdx]),
        superParameterTypes: getTokenTextsFromNode(vectorNodes[inc(vectorIdx)])
      })
      vectorIdx = vectorIdx + 2
    }
  ...
  function parseGenClassParameterTypes (vectorNode) {
    const parameterTypeNodes = getDirectFormNodesFromCollection(vectorNode)
    const parameterTypes = []
    const numParameterTypeNodes = arraySize(parameterTypeNodes)
    let idx = 0
    while (idx < numParameterTypeNodes) {
      const parameterType = getGenClassSymbolText(parameterTypeNodes[idx])
      if (!parameterType) {
        throw new Error(':gen-class :methods parameter types must be symbols inside a vector.')
      }
      stackPush(parameterTypes, parameterType)
      idx = inc(idx)
    }
  ...
  function parseGenClassMethods (vectorNode) {
    ...
    while (methodIdx < numMethodNodes) {
      const methodNode = methodNodes[methodIdx]
      ...
      stackPush(methods, {
        name,
        parameterTypes: parseGenClassParameterTypes(parameterTypesNode),
        returnType
      })
